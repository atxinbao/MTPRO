...
/// WorkbenchBetaFirstRunStateKind 定义 Workbench 启动时可展示的只读状态集合。
///
...
public enum WorkbenchBetaFirstRunStateKind: String, Codable, CaseIterable, Sendable {
...
/// WorkbenchBetaFirstRunFallbackState 是 MTP-121 的 empty / loading / error fallback 行。
///
...
public struct WorkbenchBetaFirstRunFallbackState: Codable, Equatable, Sendable {
    public let state: WorkbenchBetaFirstRunStateKind
    ...
    public init(
        state: WorkbenchBetaFirstRunStateKind,
        label: String,
        summary: String,
...
/// WorkbenchBetaFirstRunEvidenceSummary 是 Dashboard first-run 默认 demo 的只读证据摘要。
///
/// Summary 只复制 MTP-120 `WorkbenchBetaDemoFixtureEvidence` 的稳定字段：selected scenario、
...
public struct WorkbenchBetaFirstRunEvidenceSummary: Codable, Equatable, Sendable {
    ...
    public init(fixture: WorkbenchBetaDemoFixtureEvidence = .deterministicFixture) {
...
/// WorkbenchBetaFirstRunReadModel 是 MTP-121 的 App 层 first-run contract。
///
...
public struct WorkbenchBetaFirstRunReadModel: Equatable, Sendable {
    public let source: ViewModelSourceContract
    public let state: WorkbenchBetaFirstRunStateKind
    public let evidenceSummary: WorkbenchBetaFirstRunEvidenceSummary?
    public let fallbackStates: [WorkbenchBetaFirstRunFallbackState]
    ...
    public init(
        ...
        state: WorkbenchBetaFirstRunStateKind,
        evidenceSummary: WorkbenchBetaFirstRunEvidenceSummary?,
        fallbackStates: [WorkbenchBetaFirstRunFallbackState] = Self.standardFallbackStates,
        selectedScenarioID: String?,
        defaultSelectedScenarioID: String = WorkbenchBetaDemoScenarioSelection
    ...
    public static var defaultDemoState: WorkbenchBetaFirstRunReadModel {
        let summary = WorkbenchBetaFirstRunEvidenceSummary()
        return WorkbenchBetaFirstRunReadModel(
            state: .defaultDemo,
    ...
    public static var empty: WorkbenchBetaFirstRunReadModel {
        fallback(.empty)
    }

    public static func fallback(
        _ state: WorkbenchBetaFirstRunStateKind,
        summary: String? = nil
    ) -> WorkbenchBetaFirstRunReadModel {
    ...
    public static let standardFallbackStates: [WorkbenchBetaFirstRunFallbackState] = [
        WorkbenchBetaFirstRunFallbackState(
            state: .empty,
            label: "Empty",
    ...
    private static func fallbackSummary(for state: WorkbenchBetaFirstRunStateKind) -> String {
        switch state {
        case .defaultDemo:
    ...
    private static func fallbackPriority(for state: WorkbenchBetaFirstRunStateKind) -> Int {
        switch state {
        case .defaultDemo:
            0
        case .loading:
...
/// WorkbenchBetaFirstRunViewModel 是 Dashboard 可以直接绑定的 first-run 展示快照。
///
/// ViewModel 只从 `WorkbenchBetaFirstRunReadModel` 派生字段，保证 Dashboard 不直接读取
/// Core fixture、Persistence schema、Runtime object 或 Adapter request，也不提供 live command、
/// order-level command、交易按钮或真实交易授权。
public struct WorkbenchBetaFirstRunViewModel: Codable, Equatable, Sendable {
    public let source: ViewModelSourceContract
    public let state: WorkbenchBetaFirstRunStateKind
    public let stateSummary: String
    public let selectedScenarioID: String?
    ...
    public init(readModel: WorkbenchBetaFirstRunReadModel) {
        ...
        self.validationAnchors = readModel.evidenceSummary?.validationAnchors
            ?? WorkbenchBetaFirstRunEvidenceSummary.validationAnchors
