...
///
/// 这些值只是 Workbench 信息架构和合同 fixture，不是 `Command` 模型、SwiftUI 控件或 runtime
...
/// PaperWorkflowObservabilitySection 固定 Paper workflow Workbench 必须能观察的 read-model-only 区域。
///
...
/// PaperWorkflowWorkbenchContractError 描述 Workbench IA fixture 被扩大或破坏时的失败原因。
///
/// 错误只服务本地合同验证，不对外暴露 API，也不提供任何交易动作恢复路径。
public enum PaperWorkflowWorkbenchContractError: Error, Equatable, Sendable {
...
/// PaperWorkflowWorkbenchInformationArchitecture 是 MTP-47 的稳定信息架构合同。
///
...
public struct PaperWorkflowWorkbenchInformationArchitecture: Codable, Equatable, Sendable {
    ...
    public init(
        name: String = "Paper workflow Workbench control shell",
        ...
        guard source.isReadModelOnly else {
            throw PaperWorkflowWorkbenchContractError.sourceIsNotReadModelOnly
        }
        guard dashboardSections == DashboardSection.allCases else {
            throw PaperWorkflowWorkbenchContractError.dashboardSectionsMismatch
        }
        guard sessionLevelControls == Self.requiredSessionLevelControls else {
            throw PaperWorkflowWorkbenchContractError.sessionControlsMismatch
        }
        guard observabilitySections == Self.requiredObservabilitySections else {
            throw PaperWorkflowWorkbenchContractError.observabilitySectionsMismatch
        }
        guard forbiddenCapabilities == Self.requiredForbiddenCapabilities else {
            throw PaperWorkflowWorkbenchContractError.forbiddenCapabilitiesMismatch
        }
        guard allowsOrderLevelCommand == false else {
            throw PaperWorkflowWorkbenchContractError.orderLevelCommandExposed
        }
        guard implementsCommandModel == false,
              implementsUIControls == false,
              implementsEventTimeline == false
        else {
            throw PaperWorkflowWorkbenchContractError.implementationEscapedIssueScope
        }
    ...
    public static let deterministicFixture: PaperWorkflowWorkbenchInformationArchitecture = {
        do {
            return try PaperWorkflowWorkbenchInformationArchitecture()
        } catch {
            preconditionFailure("Paper workflow Workbench IA fixture must satisfy MTP-47 boundary: \(error)")
        }
    }()
}
