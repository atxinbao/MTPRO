...
MTP-195 / MTP-201 validation 必须证明 `TraderStrategyBindingsBoundaryEvidence` 固定 `Sources/Trader/StrategyBindings/` 的 generic binding protocol / coordination adapter role，当前 active concrete strategy root 只包含 EMA，OrderBookImbalance 只保留在 Core research evidence，并且 focused XCTest、automation readiness 和完整 `bash checks/run.sh` 仍通过。

## MTP-196 Trader-owned Strategy Path Validation

`MTP-196-TRADER-OWNED-STRATEGY-PATH-VALIDATION`

MTP-201 将 Trader-owned strategy path correction 固定为 EMA-only deterministic local validation。验证必须直接检查 EMA 当前 source files 位于 `Sources/Trader/Strategies/EMA/`，并确认 OrderBookImbalance active source root 已退休、`Package.swift` 只保留 `"Trader/Strategies/EMA"` 和 `"Trader/StrategyBindings"` 相关 source roots。
...
MTP-196 / MTP-201 validation 必须继续检查 `Sources/Trader/StrategyBindings/` 只包含 generic binding protocol / coordination adapter evidence。`TraderStrategyBindingsBoundaryEvidence` 是该检查的 local fixture，不允许把 EMA、OrderBookImbalance 或未来具体 strategy implementation 放入 StrategyBindings。
...
MTP-201 只执行已授权的 non-EMA active source retirement；当前仍不移动 `Sources/Trader/StrategyBindings/`，不新增 SwiftPM target、product 或 dependency，不拆 target graph，不实现 Strategy runtime、Trader runtime、ExecutionClient、OMS、broker command、signed/account endpoint、private stream runtime、Live PRO Console、trading button、live command 或 order form。
