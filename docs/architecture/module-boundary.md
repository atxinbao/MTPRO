...
- `Dashboard` 直接拥有 read model / view model display surface，只能依赖 `Core` / `Persistence` compatibility exports，不得读取 runtime object、adapter request、Database schema 或 broker payload。
...
| Dashboard | `Sources/Dashboard/` | ReadModels / Report / Events / FutureLiveProConsole / macOS shell / smoke / presentation surface；不形成 broker command path。 |
...
MTP-203 validation 的历史 EMA-only guard 已由 GH-568 的 `testReleaseActiveStrategyPathValidationRejectsNonReleaseStrategiesAndBindingDrift` 继承为 release active strategy set guard；automation readiness 和完整 `bash checks/run.sh` 继续共同覆盖。该 validation-only lineage 不移动 production source，不新增 SwiftPM target、product 或 dependency，不拆 target graph，不实现 Strategy runtime、Trader runtime、ExecutionClient、OMS、broker command、signed/account endpoint、private stream runtime、Live PRO Console、trading button、live command 或 order form。
...
MTP-207 validation 必须证明 `testMTP207TraderAccountContextValidationAnchorsCoverAccountsEMARSIAndRiskBinding`、`testMTP207TraderAccountContextValidationRejectsBrokerPayloadListenKeyAndRuntimeDrift`、automation readiness、validation plan、validation matrix 和 latest verification summary 均覆盖 account context evidence wiring；并且 `git diff --check`、`bash checks/automation-readiness.sh` 和 `bash checks/run.sh` 通过。
...
MTP-189 的 historical directory-first source migration 曾把 Workbench read-model source chain 暂放到 `Sources/Workbench/`。当前 Workbench target 已退役；ReadModels、Report evidence surfaces、Event Timeline / Evidence Explorer、Workbench beta read-model assembly、FutureLiveProConsole future label 和 macOS shell / smoke source 均由 `Sources/Dashboard/` 直接拥有。

`MTP-189-APP-COMPATIBILITY-ENVELOPE`

MTP-189 的 `App` compatibility envelope 已在后续 cleanup 中退役。当前 `Dashboard` executable target 直接编译 `Sources/Dashboard/ReadModels/`、`Sources/Dashboard/Report/`、`Sources/Dashboard/Events/`、`Sources/Dashboard/FutureLiveProConsole/`、`Sources/Dashboard/DashboardShell.swift` 和 Dashboard boundary / application entry。该收口只保持 read-model-only display surface，不等同于新增 Workbench runtime。
...
MTP-189 后旧 `Sources/App/` 不再作为 display source owner 保留。当前 `Sources/Workbench/` 也已退役；Dashboard / Report / Events 只能消费 ReadModel / ViewModel exports；`Sources/Dashboard/FutureLiveProConsole/` 只保存 future-gated label / boundary evidence，不实现 command-capable UI 或 real trading path。
...
`DomainModel` target 当前 active boundary anchor 是 `Sources/DomainModel/TargetGraph/DomainModelTargetBoundary.swift`，不依赖业务 target；canonical source root 是 `Sources/DomainModel/`。

`MTP-217-MESSAGEBUS-TARGET-SPLIT`

`MessageBus` target 当前 active boundary anchor 是 `Sources/MessageBus/TargetGraph/MessageBusTargetBoundary.swift`，只依赖 `DomainModel`。既有 `Sources/MessageBus/` 继续由 `Core` compatibility envelope 编译。

`MTP-217-DATABASE-TARGET-SPLIT`

`Database` target 当前 active boundary anchor 是 `Sources/Database/TargetGraph/DatabaseTargetBoundary.swift`，依赖 `DomainModel`、`MessageBus`、`CSQLite` 和 macOS 条件 `DuckDB`。既有 SQLite / DuckDB projection implementation 继续由 `Persistence` compatibility envelope 编译。
...
`DataClient` target 当前 active boundary anchor 是 `Sources/DataClient/TargetGraph/DataClientTargetBoundary.swift`，只依赖 `DomainModel`。既有 `Sources/DataClient/Binance/PublicMarketData/` 继续由 `Adapters` compatibility envelope 编译。

`MTP-218-CACHE-TARGET-SPLIT`

`Cache` target 当前 active boundary anchor 是 `Sources/Cache/TargetGraph/CacheTargetBoundary.swift`，依赖 `DomainModel` 和 `MessageBus`。既有 `Sources/Cache/` 继续由 `Core` compatibility envelope 编译；Cache 只表达 read-model state surface，不拥有 durable facts、Database schema、broker state 或 account payload。

`MTP-218-DATAENGINE-TARGET-SPLIT`

`DataEngine` target 当前 active boundary anchor 是 `Sources/DataEngine/TargetGraph/DataEngineTargetBoundary.swift`，依赖 `DomainModel`、`DataClient`、`MessageBus` 和 `Cache`。既有 `Sources/DataEngine/Ingest/`、`Sources/DataEngine/ScenarioReplay/` 和 `Sources/DataEngine/DataQuality/` 继续由 `Core` / `Runtime` compatibility envelope 编译。
...
`TraderStrategies` target 当前 active boundary anchor 是 `Sources/Trader/Strategies/TargetGraph/TraderStrategiesTargetBoundary.swift`，依赖 `DomainModel`、`MessageBus`、`Cache`、`Portfolio` 和 `RiskEngine`。Release v0.2.0 当前 active concrete strategies only `EMA + RSI`，canonical active root 是 `Sources/Trader/Strategies/`，active strategy paths 是 `Sources/Trader/Strategies/EMA/` 和 `Sources/Trader/Strategies/RSI/`。

`MTP-219-TRADER-TARGET-SPLIT`

`Trader` target 当前 active boundary anchor 是 `Sources/Trader/TargetGraph/TraderTargetBoundary.swift`，依赖 `DomainModel`、`MessageBus`、`Cache`、`TraderStrategies`、`Portfolio` 和 `RiskEngine`。`ExecutionEngine` target 归 MTP-220 拆分，因此本 issue 只记录 deferred dependency。

`MTP-219-PORTFOLIO-TARGET-SPLIT`

`Portfolio` target 当前 active boundary anchor 是 `Sources/Portfolio/TargetGraph/PortfolioTargetBoundary.swift`，依赖 `DomainModel`、`MessageBus`、`Cache` 和 `Database`。Portfolio 只表达 financial state projection，不拥有 Trader account identity，不读取 broker account state 或 account endpoint payload。

`MTP-219-RISKENGINE-TARGET-SPLIT`

`RiskEngine` target 当前 active boundary anchor 是 `Sources/RiskEngine/TargetGraph/RiskEngineTargetBoundary.swift`，依赖 `DomainModel`、`MessageBus`、`Cache` 和 `Portfolio`。RiskEngine 只表达 pre-execution risk boundary，不实现 live risk runtime、broker route、ExecutionClient wrapper 或 executable order command router。
...
MTP-221 曾新增 `Workbench` SwiftPM library product / target 作为 read-model consumption split 的过渡证据。当前阶段已退役 `Workbench` product / target，并把 read-model-only surface 收口到 `Dashboard` executable target。

`MTP-221-WORKBENCH-TARGET-RETIRED`

`Workbench` target / source root 当前已退役。`Sources/Workbench/`、`Sources/Workbench/TargetGraph/WorkbenchTargetBoundary.swift` 和 `Package.swift` 的 `Workbench` product / target 只能作为 historical evidence 或 forbidden active path 被引用，不再是 current compiler owner。
...
Direction 固定为 `Dashboard -> Core / Persistence read-model and ViewModel exports only`。`App` product / target、`Sources/AppCompatibility`、`Workbench` product / target、`Sources/Workbench/` 和 `WorkbenchTargetBoundary` 均已退休；`Tests/AppTests` 直接 import `Dashboard`。

`MTP-221-READ-MODEL-VIEWMODEL-ONLY`

Dashboard 只能消费已存在的 Report、Events、Evidence Explorer 和 read-model-only evidence surface，不把 UI display surface 升级为 Runtime object、Adapter request、schema access、account payload、broker payload、broker state 或 live command surface。

`MTP-221-APP-COMPATIBILITY-EXPORT-RETIRED`

`App` product / target 和 `Sources/AppCompatibility/AppCompatibility.swift` 已退休。既有 App test surface 改为直接 import `Dashboard`，不再通过 compatibility re-export 维护 `import App`。

`MTP-221-TARGETGRAPH-TEST-EVIDENCE`

`Tests/TargetGraphTests/TargetGraphTests.swift` 直接 import `Workbench` 和 `Dashboard`，验证 target split evidence、dependency direction、App compatibility retired 和 no runtime / adapter / schema / UI command drift。
...
Current target graph snapshot 固定为 `DomainModel`、`MessageBus`、`Database`、`DataClient`、`Cache`、`DataEngine`、`Portfolio`、`RiskEngine`、`ExecutionClient`、`ExecutionEngine`、`TraderStrategies`、`Trader` 和 `Dashboard`。关键 current direction 是 `Dashboard -> Core / Persistence read-model exports only`、`Trader -> ExecutionEngine` 和 `ExecutionEngine -> ExecutionClient`。

`MTP-222-HISTORICAL-COMPATIBILITY-EVIDENCE-RETAINED`

Retained `Core`、`Adapters`、`Persistence` 和 `Runtime` targets 只解释 existing implementation / import compatibility；旧 `Sources/Strategies/<strategy>`、旧 `Sources/Trader/StrategyBindings/`、`Dashboard -> App`、`App -> Core, Persistence`、`Dashboard -> Workbench` 和 `Sources/Workbench/` references 只能作为 historical / compatibility / superseded / before-state evidence 保留。Active boundary text 必须指向 `Sources/Trader/Strategies/EMA/`、`Sources/Trader/Coordination/RiskBinding/`、`TraderStrategies`、`Trader` 和 `Dashboard -> Core / Persistence read-model exports only`。

`MTP-222-NO-BEHAVIOR-RUNTIME-LIVE-GUARD`

MTP-222 不移动 production source，不新增 SwiftPM target/product/dependency，不实现 Strategy runtime、Trader runtime、Live runtime、ExecutionClient implementation、OMS implementation、broker gateway、signed endpoint、account endpoint / listenKey、private WebSocket runtime、real account read、real order lifecycle、Live PRO Console、trading button、live command、order form 或 L4 capability。后续 direct cleanup 已退休 `App` product / target 和 `Sources/AppCompatibility`。
...
MTP-223 只收口 SwiftPM target graph module split 的 validation matrix、automation readiness 和 stage audit input material。Stage audit input material 位于 `docs/audit/inputs/mtpro-swiftpm-target-graph-module-split-v1-stage-audit-input.md`。

`MTP-223-TARGET-GRAPH-CLOSEOUT`

MTP-223 closeout 必须确认 current active target graph snapshot 仍是 `DomainModel`、`MessageBus`、`Database`、`DataClient`、`Cache`、`DataEngine`、`Portfolio`、`RiskEngine`、`ExecutionClient`、`ExecutionEngine`、`TraderStrategies`、`Trader`、`Workbench` 和 `Dashboard`；retained `Core`、`Adapters`、`Persistence`、`Runtime` 和 `App` targets 只表达 existing implementation / import compatibility。

`MTP-223-NO-FINAL-STAGE-CODE-AUDIT`

MTP-223 不输出最终 Stage Code Audit Report，不设置 Linear Project `Completed`，不创建下一 Project / Issue，不推进下一阶段，不启动新的 `@002 / PAR`、Symphony 或 `symphony-issue`。Project closure、最终 Stage Code Audit Report 和 Root Docs Refresh Gate 必须由 Parent Codex closure flow 在所有 issue gates 完成后单独执行。

`MTP-223-FORBIDDEN-IMPLEMENTATION-AUDIT`

MTP-223 不实现 Strategy runtime、Trader runtime、Live runtime、ExecutionClient implementation、OMS implementation、broker gateway、signed endpoint、account endpoint / listenKey、private WebSocket runtime、real account read、real order lifecycle、Live PRO Console、trading button、live command、order form 或 L4 capability；不运行 Symphony、Graphify、code-index 或 Figma，不提交 `.codex/*` 或 `graphify-out/*`。
...
`Sources/TargetGraph` 在 MTP-224 / MTP-225 语境中只是 transitional compile anchor / historical evidence。MTP-231 后它不再是 active source directory 或 active target path reference；历史 `Sources/TargetGraph/<Module>` 文字只能作为 before-state / retired evidence 保留。它不代表最终 architecture module、长期 source ownership、新 engine layer 或未来 feature landing path。
...
当前 active target roots 固定为 `Sources/DomainModel`、`Sources/MessageBus`、`Sources/Database`、`Sources/DataClient`、`Sources/Cache`、`Sources/DataEngine`、`Sources/Trader/Strategies/EMA`、`Sources/Trader`、`Sources/Portfolio`、`Sources/RiskEngine`、`Sources/ExecutionClient`、`Sources/ExecutionEngine` 和 `Sources/Dashboard`。这些 roots 继续保留 MTP-222 dependency direction 和 retained compatibility implementation boundary，不引入 runtime、live、broker 或 L4 capability。
...
| Dashboard / Report / Events | `Sources/Dashboard/` | 只能消费 ReadModel / ViewModel；不读 runtime object、adapter request、schema 或 broker payload。 |
...
| `Dashboard` Report / Events | `Sources/Dashboard/` | UI 只能消费 ReadModel / ViewModel；不得读取 runtime object、adapter request、schema 或 broker payload。 |
| `Dashboard` smoke / fixtures | `Sources/Dashboard/` | 保留为可观察 evidence surface，不形成 live command surface。 |
