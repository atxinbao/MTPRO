...
| Trader | `Sources/Trader/` | `Accounts`、`Strategies`、`Coordination`、`StrategyBindings`；只做 strategy ownership、coordination / context。 |
...
MTP-193 保留现有 `Core` SwiftPM product / target 名称作为 compatibility envelope，但 `Package.swift` 的 EMA source root 必须从 `"Strategies/EMA"` 更新为 `"Trader/Strategies/EMA"`。该变更不新增 SwiftPM target、product 或 dependency，不做 target graph split；MTP-194 后 OrderBookImbalance 也通过 `Sources/Trader/Strategies/OrderBookImbalance/` 编译。
...
MTP-194 保留现有 `Core` SwiftPM product / target 名称作为 compatibility envelope，但 `Package.swift` 的 OrderBookImbalance source root 必须从 `"Strategies/OrderBookImbalance"` 更新为 `"Trader/Strategies/OrderBookImbalance"`。该变更不新增 SwiftPM target、product 或 dependency，不做 target graph split。
...
MTP-196 将 Trader-owned strategy path correction 固定为 deterministic local validation。验证必须直接检查 EMA 和 OrderBookImbalance 当前 source files 位于 `Sources/Trader/Strategies/<strategy>/`，并确认 `Package.swift` 使用 `"Trader/Strategies/EMA"` 和 `"Trader/Strategies/OrderBookImbalance"`。
...
MTP-196 validation 必须继续检查 `Sources/Trader/StrategyBindings/` 只包含 generic binding protocol / coordination adapter evidence。`TraderStrategyBindingsBoundaryEvidence` 是该检查的 local fixture，不允许把 EMA、OrderBookImbalance 或未来具体 strategy implementation 放入 StrategyBindings。
...
MTP-196 只添加 validation evidence、docs anchors 和 automation readiness checks；不移动 source files，不修改 `Package.swift` source roots，不新增 SwiftPM target、product 或 dependency，不做 target graph split。
...
`RSI`、`OrderBookImbalance`、`Momentum` 和 `MeanReversion` 只能作为 future strategy candidate / future-gated strategy label。现有 `Sources/Trader/Strategies/OrderBookImbalance/` 是 MTP-194 历史迁移后留下的 compatibility / superseded source placement debt，当前不得再写成 active concrete strategy；后续由 MTP-200 audit 和 MTP-201 retirement / quarantine issue 处理。

`MTP-198-STRATEGYBINDINGS-NOT-FIRST-LEVEL-STRATEGY-DIRECTORY`

`Sources/Trader/StrategyBindings/` 不是 first-level Trader strategy directory，不是 concrete strategy implementation landing path，也不是 EMA、RSI、OrderBookImbalance、Momentum 或 MeanReversion 的源码落点。StrategyBindings 仍只表达 generic binding protocol / coordination adapter contract。

`MTP-198-TRADER-COORDINATION-BINDING-RESPONSIBILITY`

Binding / adapter semantics 归 `Sources/Trader/Coordination/` 责任边界管理。需要连接 strategy instance、account context、RiskEngine、Portfolio evidence 或 ExecutionEngine evidence 时，应由 `Trader/Coordination/<binding>/` 这类 coordination boundary 表达，例如 future `Trader/Coordination/RiskBinding/`；MTP-198 不创建该目录、不迁移 source、不实现 runtime。
...
MTP-198 不移动 `Sources` 文件，不删除 `Sources/Trader/Strategies/OrderBookImbalance/`，不移动 `Sources/Trader/StrategyBindings/`，不修改 `Package.swift`，不新增 SwiftPM target、product 或 dependency，不拆 target graph，不写 Swift business code，不实现 Strategy runtime、Trader runtime、ExecutionClient、OMS、broker command、signed/account endpoint、private stream runtime、Live PRO Console、trading button、live command 或 order form。
...
MTP-191 将 MTP-187 的 `Sources/Strategies/EMA/` 与 `Sources/Strategies/OrderBookImbalance/` 降级为 compatibility / superseded physical location。MTP-193 已把 EMA concrete strategy files 迁入 `Sources/Trader/Strategies/EMA/`；MTP-194 已把 OrderBookImbalance concrete strategy files 迁入 `Sources/Trader/Strategies/OrderBookImbalance/`。

`MTP-187-STRATEGIES-COMPATIBILITY-ENVELOPE`

MTP-187 保留现有 `Core` SwiftPM product / target 名称作为 Strategies 迁移期兼容外壳。MTP-194 后 `Core` target 继续编译 `Sources/Trader/Strategies/EMA/` 和 `Sources/Trader/Strategies/OrderBookImbalance/`，让既有 tests 和 downstream target 仍通过 `import Core` 使用 strategy signal / proposal 类型。该兼容壳只保持 buildability，不等同于 Strategy runtime 或最终 target graph split。
...
| Strategies | `Sources/Trader/Strategies/<strategy>/` | Trader-owned concrete strategy definitions；MTP-193 后 EMA 当前位于 `Sources/Trader/Strategies/EMA/`，MTP-194 后 OrderBookImbalance 当前位于 `Sources/Trader/Strategies/OrderBookImbalance/`；具体策略仍不能直连 ExecutionClient。 |
| Trader | `Sources/Trader/` | account / strategy / risk / execution coordination boundary；包含 `Accounts`、`Strategies`、`Coordination` 和 `StrategyBindings`，当前只允许 identity / lifecycle / read-model input。 |
