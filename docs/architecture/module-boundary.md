...
MTP-191 将 strategy directory rule 修正为 Trader-owned form：后续 canonical rule 是 `DataClient/<venue>/` 和 `Trader/Strategies/<strategy>/`。旧 `Strategies/<strategy>/` 只作为已完成迁移的历史/兼容路径；MTP-193 后 EMA 已迁入 Trader-owned path，OrderBookImbalance 仍等待 MTP-194 执行源码迁移。
...
MTP-193 保留现有 `Core` SwiftPM product / target 名称作为 compatibility envelope，但 `Package.swift` 的 EMA source root 必须从 `"Strategies/EMA"` 更新为 `"Trader/Strategies/EMA"`。该变更不新增 SwiftPM target、product 或 dependency，不做 target graph split；OrderBookImbalance 在 MTP-194 前仍通过 `Sources/Strategies/OrderBookImbalance/` 编译。
...
MTP-191 将 MTP-187 的 `Sources/Strategies/EMA/` 与 `Sources/Strategies/OrderBookImbalance/` 降级为 compatibility / superseded physical location。MTP-193 已把 EMA concrete strategy files 迁入 `Sources/Trader/Strategies/EMA/`；`Sources/Strategies/OrderBookImbalance/` 仍是 MTP-194 的待迁移来源。

`MTP-187-STRATEGIES-COMPATIBILITY-ENVELOPE`

MTP-187 保留现有 `Core` SwiftPM product / target 名称作为 Strategies 迁移期兼容外壳。MTP-193 后 `Core` target 继续编译 `Sources/Trader/Strategies/EMA/` 和 `Sources/Strategies/OrderBookImbalance/`，让既有 tests 和 downstream target 仍通过 `import Core` 使用 strategy signal / proposal 类型。该兼容壳只保持 buildability，不等同于 Strategy runtime 或最终 target graph split。
...
| Strategies | `Sources/Trader/Strategies/<strategy>/` | Trader-owned concrete strategy definitions；MTP-193 后 EMA 当前位于 `Sources/Trader/Strategies/EMA/`，`Sources/Strategies/OrderBookImbalance/` 仍是 MTP-194 前的 compatibility / superseded location；具体策略仍不能直连 ExecutionClient。 |
