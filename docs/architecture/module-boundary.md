...
MTP-191 将 strategy directory rule 修正为 Trader-owned form：后续 canonical rule 是 `DataClient/<venue>/` 和 `Trader/Strategies/<strategy>/`。旧 `Strategies/<strategy>/` 只作为已完成迁移的历史/兼容路径，直到 MTP-193 / MTP-194 执行源码迁移。
...
MTP-191 将 MTP-187 的 `Sources/Strategies/EMA/` 与 `Sources/Strategies/OrderBookImbalance/` 降级为 compatibility / superseded physical location。MTP-193 / MTP-194 才允许把这些 concrete strategy files 迁入 `Sources/Trader/Strategies/EMA/` 和 `Sources/Trader/Strategies/OrderBookImbalance/`。
...
| Strategies | `Sources/Trader/Strategies/<strategy>/` | Trader-owned concrete strategy definitions；当前 MTP-187 的 `Sources/Strategies/EMA/` 和 `Sources/Strategies/OrderBookImbalance/` 是 compatibility / superseded location，后续迁入 Trader-owned path；具体策略仍不能直连 ExecutionClient。 |
