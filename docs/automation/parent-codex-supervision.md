...
| Parent Codex Automation Supervision | 核对 Project / Issue 执行合同格式，做 queue preview，在 Human 授权后操作唯一 `Backlog` -> `Todo`，监督 child Codex、host-side fallback 和 Stage Code Audit | 不默认写业务代码，不创建新 Project / Issue，不决定下一阶段目标，不直接 merge PR |
...
6. 更新 pointer 后执行只读 queue preview。
...
Parent Codex 更新 pointer 后必须先做 queue preview，不得直接启动 `symphony-issue`，不得直接操作 `Backlog` -> `Todo`。
