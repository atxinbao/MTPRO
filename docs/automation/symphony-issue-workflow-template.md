...
- Human authorized first issue Todo：no
...
5. 执行只读 queue preview。
6. 核对 WIP=1。
7. 核对 issue execution contract。
8. 等待 Human 明确授权某个 issue 进入 `Todo`。
9. Parent Codex 执行 `Backlog` -> `Todo`。
...
但当前仍无 `Todo` issue，`MTP-16` 仍需 Human 明确授权后，才可由 Parent Codex 推进为唯一 `Todo`。
