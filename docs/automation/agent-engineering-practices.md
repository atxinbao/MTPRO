...
它是流程约束和 review 依据，不是 Linear Project，不是 issue body，不授权执行，不推进 `Todo`，不启动额外调度服务，不写业务代码。

本文档只吸收 `mattpocock/skills` 的工程方法，不安装、不调用、不复制外部 skill runtime。MTPRO 的正式执行入口仍是 Linear live-read、Parent Codex queue preflight、唯一 configured executable issue、Codex Execution Agent、GitHub PR Automation、Stage Code Audit 和 Root Docs Refresh Gate。
...
| Handoff | PR evidence、Post-Issue Ledger、Stage Audit、latest verification summary |
...
- `.codex/*` 未进入 PR。
- 如由 host-side fallback 执行，提供 handoff / ledger evidence。
