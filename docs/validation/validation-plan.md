...
- Focused verifier: `bash checks/verify-v0.21.0-spot-canary-environment-profile.sh`
- Focused test: `swift test --filter TargetGraphTests/testGH1274ReleaseV0210SpotCanaryEnvironmentProfile`
...
- Boundary: GH-1274 defines only the v0.21.0 Binance Spot canary environment profile. It records productionLive identity only, default-off fail-closed policy and explicit Human operator opt-in evidence requirement. It does not read production secret, connect production endpoint / broker endpoint, implement signed account endpoint runtime, submit / cancel / replace, publish tag / GitHub Release, include Futures / OKX, or authorize production cutover.
...
- Focused verifier: `bash checks/verify-v0.21.0-credential-secret-read-approval.sh`
- Focused test: `swift test --filter TargetGraphTests/testGH1275ReleaseV0210CredentialSecretReadApprovalPath`
...
- Focused verifier: `bash checks/verify-v0.21.0-live-account-snapshot-redaction.sh`
- Focused test: `swift test --filter TargetGraphTests/testGH1277ReleaseV0210LiveAccountSnapshotRedactionArtifact`
...
- Boundary: GH-1277 defines only the v0.21.0 Binance Spot redacted live account snapshot artifact and freshness evidence. It consumes GH-1276 signed account read-only preflight evidence, keeps only readiness / freshness redacted fields, rejects stale or malformed snapshots, and does not persist raw balances, account id, raw account payload, touch order endpoint, submit / cancel / replace, publish tag / GitHub Release, include Futures / OKX, or authorize production cutover.
...
- Focused verifier: `bash checks/verify-v0.21.0-canary-hard-limits.sh`
- Focused test: `swift test --filter TargetGraphTests/testGH1278ReleaseV0210CanaryHardLimitPreTradeGate`
...
- Focused verifier: `bash checks/verify-v0.21.0-controlled-spot-canary-submit.sh`
- Focused test: `swift test --filter TargetGraphTests/testGH1280ReleaseV0210ControlledSpotCanarySubmitPath`
...
- Focused verifier: `bash checks/verify-v0.21.0-canary-oms-event-log-reconciliation.sh`
- Focused test: `swift test --filter TargetGraphTests/testGH1282ReleaseV0210CanaryOMSEventLogReconciliationEvidence`
...
- Focused verifier: `bash checks/verify-v0.21.0-canary-operator-runbook.sh`
...
## GH-1244 Release v0.20.0 Signed Account Read-only Readiness
...
- Focused verifier: `bash checks/verify-v0.20.0-stage-audit-release-docs.sh`
- Focused test: `swift test --filter TargetGraphTests/testGH1250ReleaseV0200StageAuditReleaseDocsCloseout`
...
- Boundary: GH-1250 只收口 #1239..#1250 construction evidence、Stage Code Audit、release notes、validation matrix、root docs refresh 和 release publication gate handoff。它不运行 Spot canary，不创建 tag / GitHub Release，不读取 production secret，不连接 production endpoint / broker endpoint，不触达 account / order endpoint，不提交 / 取消 / 替换订单，不授权 production cutover。production cutover not authorized。
...
- Evidence files: `Sources/ExecutionClient/FutureGate/ReleaseV0170ManualWorkflowArtifactValidation.swift`、`docs/history/workflows/release-v0.17.0-manual-artifact-validation.yml`、`docs/contracts/release-v0.17.0-manual-workflow-artifact-validation-contract.md`、`README.md`、`GOAL.md`、`BLUEPRINT.md`、`docs/roadmap.md`、`docs/validation/latest-verification-summary.md`、`docs/validation/trading-validation-matrix.md`、`docs/automation/automation-readiness.md`、`checks/verify-v0.17.0-manual-workflow-artifact-validation.sh`、`checks/run.sh`、`checks/automation-readiness.sh` 和 `Tests/TargetGraphTests/TargetGraphTests.swift`。
...
- Boundary: GH-1112 只收口 v0.16.0 Stage Code Audit、release notes、operator runbook、validation matrix 和 stale wording guard；#1112 closeout 本身不创建 tag / GitHub Release，不推进下一 Project / Issue，不授权 production cutover，不读取 production secret，不连接 production endpoint / broker endpoint，不提交 production order。后续独立 Release Publication Gate 已发布 v0.16.0 stable GitHub Release：`https://github.com/atxinbao/MTPRO/releases/tag/v0.16.0`，tag peeled commit `28779236262bd7ffaf71e286b27b95854c5cd3e1`；该 publication 仍不授权 production cutover。
...
- Required command: `bash checks/verify-v0.16.0-cli-cancel-flow.sh`
- Focused test: `testGH1104ReleaseV0160CLICancelFlowConsumesSubmitArtifactAndFailsClosed`
...
- Required command: `bash checks/verify-v0.15.1-v0150-release-fact-sync.sh`
- Focused test: `testGH1094ReleaseV0151V0150ReleaseFactSyncGuardRejectsStalePublicationWording`
...
- Required command: `bash checks/verify-v0.15.0-real-spot-testnet-cancel-runtime.sh`
- Focused test: `testGH1069ReleaseV0150SpotTestnetCancelRuntimeAppendsRedactedCancelEvidence`
- Evidence files: `Sources/ExecutionClient/FutureGate/ReleaseV0150BinanceSpotTestnetCancelRuntime.swift`、`Sources/ExecutionClient/FutureGate/ReleaseV0150BinanceSpotTestnetNetworkExecutionEventLog.swift`、`Sources/ExecutionClient/FutureGate/ReleaseV0150BinanceSpotTestnetSubmitRuntime.swift`、`Sources/ExecutionClient/FutureGate/ReleaseV0150BinanceSpotTestnetSignedRequestBuilder.swift`、`docs/contracts/release-v0.15.0-real-spot-testnet-cancel-runtime-contract.md`、`checks/verify-v0.15.0-real-spot-testnet-cancel-runtime.sh`、`checks/run.sh`、`checks/automation-readiness.sh`、`docs/automation/automation-readiness.md`、`docs/validation/latest-verification-summary.md` 和 `Tests/TargetGraphTests/TargetGraphTests.swift`。
- Boundary: GH-1069 只允许 Binance Spot Testnet cancel runtime 消费已完成的 #1068 submit runtime evidence、redacted testnet order identity reference、显式 operator confirmation、注入式 Spot Testnet cancel transport 和 #1071 append-only network event log；`httpMethod=DELETE`，`testnetNetworkCancelPerformed=true`，`appendOnlyCancelEvidenceCreated=true`，`omsStateTransitionIntegrated=true`，`productionTradingEnabledByDefault=false`，`productionSecretAutoRead=false`，`productionEndpointConnected=false`，`brokerEndpointConnected=false`，`productionOrderSubmitted=false`；不授权 production cutover、cancel-replace、Dashboard command surface、broker fill 或 production order。
...
- Required command: `bash checks/verify-v0.11.1-dashboard-macos-v0110-guards.sh`
- Focused tests: `testGH919DashboardProductionReadinessCenterBindsRealLocalArtifactStatesReadOnly`、`testGH919DashboardProductionReadinessCenterBindsRealArtifactStateAnchors` 和 `testGH946DashboardMacOSV0110GuardRunsReadinessArtifactStateBeforeBuildAndSmoke`
- Evidence files: `.github/workflows/checks.yml`、`checks/verify-v0.11.1-dashboard-macos-v0110-guards.sh`、`checks/verify-v0.11.0.sh`、`checks/run.sh`、`Sources/Dashboard/Report/ReleaseV0100DashboardProductionReadinessCenter.swift`、`Sources/Dashboard/DashboardShell.swift`、`Tests/AppTests/AppTests.swift` 和 `Tests/TargetGraphTests/TargetGraphTests.swift`。
- Boundary: GH-946 只让 required `dashboard-macos` job 在 Dashboard build / smoke 前执行 v0.11 verifier 与 Dashboard readiness artifact state focused guard；不新增 trading button、order form、live command、submit / cancel / replace、production endpoint / broker、production secret read、testnet order routing 或 production cutover。
...
## GH-1340 Release v0.22.1 Patch Audit / Release Notes Closeout
...
## GH-1350 Release v0.23.0 Aggregate Validation Suite
...
## Release v0.26.0 Validation Plan
...
Run `bash checks/verify-v0.26.0.sh` plus the full `bash checks/run.sh`. The validation suite must preserve the Binance USD-M Futures testnet controlled execution foundation, `productionFuturesOrderExecutionEnabled=false`, production cutover not authorized, read-only Dashboard / CLI surfaces, manual approval hard caps, redaction, idempotency and no OKX active runtime.

## Release v0.26.1 Publication Fact Sync / Milestone Closure Patch

Validation anchors: `GH-1406-VERIFY-V0261-V0260-RELEASE-FACT-SYNC`, `TVM-RELEASE-V0261-V0260-RELEASE-FACT-SYNC`, `V0261-001-V0260-GITHUB-RELEASE-PUBLISHED`, `V0261-001-V0260-TAG-FIXED`, `V0261-001-V0260-PUBLISHED-AT-2026-07-08T13-00-01Z`, `GH-1407-VERIFY-V0261-V0260-MILESTONE-COMPLETION`, `V0261-002-V0260-MILESTONE-CLOSED`, `V0261-002-V0260-ISSUES-1394-1403-DONE`, `GH-1408-VERIFY-V0261-V0260-STALE-WORDING-GUARD`, `V0261-003-PUBLISHED-V0260-STALE-WORDING-GUARD`, `GH-1409-VERIFY-V0261-V0260-BASELINE-WORDING`, `V0261-004-V0260-CURRENT-PUBLISHED-BASELINE`, `V0261-004-FUTURES-TESTNET-CONTROLLED-EXECUTION-FOUNDATION`, `GH-1410-VERIFY-V0261-PATCH-AUDIT-RELEASE-NOTES`, `V0261-005-PATCH-AUDIT`, `V0261-005-V0270-BLOCKED-BY-V0261-COMPLETION`, `V0261-005-NO-CAPABILITY-CHANGE`.

Run `swift test --filter TargetGraphTests/testGH1406To1410ReleaseV0261PublicationFactSyncMilestoneClosurePatch`, `bash checks/verify-v0.26.1.sh`, and the full `bash checks/run.sh`. The suite records the published v0.26.0 GitHub Release at https://github.com/atxinbao/MTPRO/releases/tag/v0.26.0, tag target `e3b65f2337c5275eaa7ce5c5f224b69475a7c9bb`, publication timestamp `2026-07-08T13:00:01Z`, v0.26.0 milestone #43 closed with 0 open / 10 closed issues, and current maturity `Binance USD-M Futures testnet controlled execution foundation`. production cutover not authorized. v0.27.0 remains blocked until v0.26.1 completion.
...
Required commands: `swift test --filter TargetGraphTests/testGH1468To1475ReleaseV0300ObservedProductionShadowRun`, `bash checks/verify-v0.30.0.sh`, `bash checks/verify-v0.30.1.sh`, `git diff --check`, `bash checks/automation-readiness.sh`, `bash checks/run.sh`. Deterministic fixture evidence must remain `evidenceOrigin=deterministic-fixture`, `acceptanceDecision=blocked`, `observedRunAccepted=false`; explicit artifact-root manifest validation is required for acceptance.
...
Required commands: `swift test --filter TargetGraphTests/testGH1487To1496ReleaseV0310ControlledProductionEnablementGate`, `bash checks/verify-v0.31.0.sh`, `git diff --check`, `bash checks/automation-readiness.sh`, `bash checks/run.sh`. Required facts: `decision=blocked`, `productionTradingEnabledByDefault=false`, `productionCutoverAuthorized=false`, `automaticSecretReadEnabled=false`, `automaticBrokerConnectionEnabled=false`, `productionSubmitCancelReplaceEnabled=false`.
