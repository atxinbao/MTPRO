...
- flat position + `targetLong` 必须生成本地 BUY open-long intent。
- flat position + `targetShort` 必须生成本地 SELL open-short intent。
- long position + `targetFlat` 必须生成 reduce-only SELL close-long intent。
- short position + `targetFlat` 必须生成 reduce-only BUY close-short intent。
- targetLong from short 和 targetShort from long 必须 blocked，不能自动合成 close + open 的单步翻仓。
- 生成的 order intent 仍必须保留 OMS、Event Store、kill switch 和 no-trade 后续 gate，不授权真实 submit / cancel / replace、leverage action 或 margin action。

GH-582 必须建立的主要 anchors：
...
- 不读取、打印、保存或推导 production secret。
- 不连接 production endpoint、production broker endpoint、signed endpoint、account endpoint、private stream endpoint 或 listenKey。
- 不把 Perp ExecutionAlgorithm order intent 解释为真实订单、broker request、OMS command、ExecutionClient request、leverage / margin action 或 production authorization。
- 不自动执行 close + open 的单步翻仓。
- 不启用 non-Binance venue。
- 不启用非 Spot / USDⓈ-M Perpetual product。
- 不启用非 EMA / RSI active strategy。
- 不绕过 CommandGateway、RiskEngine、ExecutionEngine、OMS、Event Store、kill switch 或 no-trade gate。
...
- Spot adapter 必须只消费 GH-583 Spot OMS handoff identity，不能直接依赖 `ExecutionEngine` target。
- adapter 必须覆盖 Binance Spot submit / cancel / replace 三个 command kind。
- dry-run preview 必须完整输出 request mapping evidence，且不得执行 network call。
- testnet request mapping 必须固定到 `https://testnet.binance.vision`，submit / cancel 使用 `/api/v3/order`，replace 使用 `/api/v3/order/cancelReplace`。
- query item names 必须与 Binance Spot testnet mapping 对齐，并机械拒绝 `signature` item。
- deterministic ack 必须与 requestID 保持一一对应，且不得被解释为 broker fill、execution report、reconciliation 或真实订单结果。
- 生成的 adapter evidence 必须保持 production rejected by default，不授权 production endpoint、production secret、broker gateway、Dashboard live command surface 或真实 submit / cancel / replace。

GH-584 必须建立的主要 anchors：
...
- 不实现 Binance USDⓈ-M Perpetual adapter；Perp 适配器必须留给 GH-585。
- 不读取、打印、保存或推导 production secret。
...
- Projection 必须消费 GH-586 Perp normalized BrokerFill / position update evidence 和 GH-575 fresh mark / funding read model；Spot BrokerFill 进入 Perp projection 必须被拒绝。
- Perp fill 必须更新 positionAmt、entryPrice、markPrice、margin requirement、本地 realized / unrealized / net PnL、funding estimate 和 EMA / RSI strategy attribution。
- Projection 必须产出 Portfolio financial state projection evidence，但不得让 `Portfolio` target 直接依赖 `ExecutionClient`。
- 生成的 projection evidence 必须保持 production trading disabled by default，不授权 production account endpoint、production secret、broker gateway、broker position sync、leverage / margin action、funding settlement、reconciliation runtime、Portfolio runtime mutation、Dashboard live command surface 或真实 submit / cancel / replace。

GH-588 必须建立的主要 anchors：
...
- 不执行真实 submit / cancel / replace，不把 Perp Portfolio projection evidence 解释为 production fill、production position、broker statement、funding settlement 或 production authorization。
- 不执行 reconciliation runtime，不同步 broker position，不读取真实 balance、real PnL、margin、leverage、buying power 或 account endpoint payload。
...
- `Database` target 必须显式编译 `Sources/Database/ReleaseV020VerificationGates.swift`，并保持该 verification gate 不进入 Persistence / Runtime compatibility owner。
- `verify-fast` 必须覆盖 `foundation` 和 `sample-traces` section。
- `verify-fast` 的 sample traces 必须覆盖 Spot market data、Perp market data、EMA Spot / Perp target exposure、RSI Spot / Perp target exposure。
- `verify-release` 必须覆盖 `foundation`、`sample-traces`、`full-gates` 和 `all-traces` section。
- `verify-release` 必须覆盖 GH-592 的 15 条 golden trace catalog。
- `mtpro verify-fast` 和 `mtpro verify-release` 必须输出 `verificationIssue=GH-595`、`verifyCoverage=*`、`verifyTraceCount=*`、`verifyCatalogTraceCount=15` 和 `verifyGateBoundaryHeld=true`。
- Evidence 必须拒绝 production trading default、production secret read、production endpoint touch、broker gateway touch、account endpoint read、real submit / cancel / replace 和 gate bypass。

GH-595 必须建立的主要 anchors：
...
- 不读取、打印、保存或推导 production secret。
- 不连接 production endpoint、production broker endpoint、signed endpoint、account endpoint、listenKey 或 private stream endpoint。
- 不把 verify-fast / verify-release 扩大成 production runbook、operator authorization、production replay job、broker statement、account checkpoint、reconciliation job 或 production cutover。
- 不执行真实 submit / cancel / replace，不把 `mtpro verify-fast` 或 `mtpro verify-release` 解释为 production trading authorization。
- 不执行 production reconciliation runtime，不同步 broker position，不读取真实 balance、real PnL、margin、leverage、buying power 或 account endpoint payload。
- 不绕过 CommandGateway、RiskEngine、ExecutionEngine、OMS、Event Store、kill switch 或 no-trade gate。
- 不启用 non-Binance venue。
