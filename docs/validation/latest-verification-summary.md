...
| 最近完成 Linear Project | `MTPRO Event-Driven Paper Trading Runtime v1` |
| Current planning record | `MTPRO Data Catalog / Scenario Replay v1` planning record 已落仓；不授权 execution，后续执行必须经 Linear 写入和 Parent Codex queue preflight |
| 最近完成 Linear Project status | Linear Project status `Completed`，`type=completed`，state `completed`，`completedAt=2026-05-25T18:25:12.000Z` |
| Stage Code Audit Report | `docs/audit/mtpro-event-driven-paper-trading-runtime-v1-stage-code-audit.md`，已覆盖完整 Linear Project |
...
`MTPRO Data Catalog / Scenario Replay v1` Project-level planning record 已落仓，路径为 `docs/planning/projects/mtpro-data-catalog-scenario-replay-v1-plan.md`。该文档只保存 Project 级计划摘要和格式门槛，不复制维护完整 Linear issue body 或 candidate issue 正文；它不创建 Linear Project，不创建 Linear Issues，不修改 Linear status，不推进 Todo，不启动 `@002 / PAR`，不启动 Symphony，不运行 Graphify update，不修改 Figma，不写业务代码，不实现 Data Catalog，不实现 Scenario Replay。该 planning record 的 Target Engines 为 Data Engine、State & Persistence Engine 和 Workbench Interface，Target maturity 为 `L1.5 -> L2 prerequisite`，目标是为后续 Simulated Exchange / Backtest Parity、Workbench beta demo path 和 report reproducibility 建立 local-first、deterministic、versioned scenario replay 数据地基。它不是 Project closure，不更新 Final Product Goal Progress，不授权 implementation；后续执行必须先写入 Linear，并由 Parent Codex queue preflight 在 WIP=1、依赖满足、无 active conflict 和 execution contract 格式完整时推进唯一 eligible issue。
