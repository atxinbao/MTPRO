...
| 命令 | 结果 | 说明 |
| --- | --- | --- |
| `git diff --check` | pass | 由 `bash checks/run.sh` 串联执行；覆盖 MTP-44 变更与当前 main reference study 文档合并状态。 |
...
| `swift test` | pass | 93 个 XCTest，0 failures；覆盖 MTP-44 后完整 Core / Persistence / Runtime / App 回归。 |
