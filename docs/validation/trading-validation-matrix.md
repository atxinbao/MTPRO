...
| `MTP-37` | `TVM-PAPER-SESSION-LIFECYCLE`、`TVM-PAPER-ACTION-PROPOSAL`、`TVM-RISK-BLOCKER`、`TVM-PORTFOLIO-EXPOSURE`、`TVM-PAPER-SESSION-REPLAY`、`TVM-REPORT-EVIDENCE` | 已新增 `docs/validation/mtp-37-stage-audit-input.md`，集中记录 MTP-31 至 MTP-36 的 PR evidence、merge commit、required check、paper runtime validation evidence chain、known boundaries、automation readiness evidence 和 Stage Code Audit handoff checklist；最终 Stage Code Audit Report 仍由 Parent Codex 在 Project 全部 Done 后单独输出。 |
...
| 收口项 | Evidence location | 审计用途 |
| --- | --- | --- |
| Issue / PR evidence | `docs/validation/mtp-37-stage-audit-input.md` 的 `Issue / PR evidence input` | 为 Parent Codex 汇总 PR #62、#63、#64、#65、#66、#67 和 MTP-37 PR 提供输入。 |
| Paper runtime evidence chain | `docs/validation/mtp-37-stage-audit-input.md` 的 `Paper runtime validation evidence chain` | 确认 lifecycle、proposal、risk blocker、portfolio exposure、replay 和 report evidence 均有 test / fixture / read model / PR evidence。 |
| Known boundaries | `docs/validation/mtp-37-stage-audit-input.md` 的 `Known boundaries` | 为 Stage Code Audit 的 paper-only、Live trading、signed endpoint、broker action、schema leakage 和 Report 授权边界提供输入。 |
| Automation readiness | `checks/automation-readiness.sh`、`docs/automation/verified-operations.md`、`.github/pull_request_template.md` | 确认 `checks`、WIP=1、handoff marker、Post-Issue Ledger、Graphify ignore 和 PR Automation 证据链仍完整。 |
| Root Docs Delta input | `docs/validation/mtp-37-stage-audit-input.md` 的 `Root Docs Delta input` | 提醒 Parent Codex 在最终 Stage Code Audit Report 中检查 root docs，只同步已发生事实。 |
