...
| `TVM-PAPER-EXECUTION-WORKFLOW` | Paper-only execution workflow contract 和事件边界 | `Sources/Core/PaperExecutionWorkflowContract.swift` 中的 `PaperExecutionWorkflowStage`、`PaperExecutionWorkflowEvidenceKind`、`PaperExecutionWorkflowStageBoundary`、`PaperExecutionWorkflowContract.deterministicFixture`；`Tests/CoreTests/CoreTests.swift` 中的 `testPaperExecutionWorkflowContractDefinesPaperOnlyStageAndEventBoundaries` 和 `testPaperExecutionWorkflowContractRejectsRealTradingCapabilityAndOrderBypass`。 | 证据覆盖 proposal -> risk decision -> paper execution decision -> paper order -> simulated fill -> portfolio projection 的阶段顺序、`.paper` / `.risk` / `.portfolio` event stream 边界、future issue 占位、Codable 禁区和 paper-only capability flags；不得实现 paper order lifecycle、simulated fill、完整 OMS、signed endpoint、broker action 或真实订单行为。 | MTP-38 已回填 workflow contract、deterministic fixture、Core tests、validation 摘要和 PR evidence 边界；后续 MTP-39 至 MTP-45 只能在该 stage / event boundary 内补充本地 paper-only evidence，不得绕过 risk decision 或恢复真实交易能力。 |
