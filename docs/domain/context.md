...
`Trader target boundary` 指 `Accounts + Strategies/EMA + Coordination` container 的 target。它可以消费 `TraderStrategies`、`Portfolio` 和 `RiskEngine` evidence/context，但不能直连 `ExecutionClient`、broker、OMS、signed/account endpoint、listenKey 或 UI command surface。`Trader -> ExecutionEngine` 的 target dependency 延后到 MTP-220。
