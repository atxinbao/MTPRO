...
`OrderBookImbalance research evidence retirement` 指 order-book imbalance research source 已从 MTP-194 的 Trader-owned non-EMA path `Sources/Trader/Strategies/OrderBookImbalance/` 退休到 `Sources/Core/Research/OrderBookImbalanceResearchEvidence.swift`。MTP-201 后该 evidence 只表达 historical research / parity / persistence support，不是当前 active concrete strategy；当前 active concrete strategy 只有 EMA。该术语不改变 imbalance calculation、bias semantics、input source evidence、fixtures 或 `import Core` compatibility surface。
...
`TraderStrategies target boundary` 指只承载 Trader-owned concrete strategy definitions 的 target。当前 active concrete strategy only `EMA`，canonical active source root only `Sources/Trader/Strategies/EMA/`；非 EMA strategy 只能作为 future candidate / historical evidence / compatibility debt，不得成为 active target source。
...
`Real module source root` 指后续 active target path 的真实目标目录，例如 `Sources/DomainModel/`、`Sources/MessageBus/`、`Sources/DataClient/`、`Sources/DataEngine/`、`Sources/Trader/Strategies/EMA/`、`Sources/Trader/Accounts/`、`Sources/Trader/Coordination/`、`Sources/ExecutionEngine/`、`Sources/ExecutionClient/` 和 `Sources/Dashboard/`。当前 active concrete strategy only `EMA`；后续多个策略应放在 `Sources/Trader/Strategies/<strategy>/`。`Sources/Workbench/` 当前已退役，只能作为 historical / forbidden active path evidence。
