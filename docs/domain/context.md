...
| `DataClient` | exchange-scoped client boundary；Release v0.2.0 当前 active venue 固定 Binance，active product types 固定 Spot + USDⓈ-M Perpetual，production secret / endpoint 默认关闭 | 不等于 execution adapter，不绕过 production gates，不调用非授权 account endpoint 或 listenKey |
...
| `ExecutionClient` | gated exchange / broker execution client capability boundary | Release v0.2.0 只允许对应 issue scope 内推进 Binance Spot / USDⓈ-M Perpetual testnet / dry-run evidence；production capability 默认关闭且受 gate 约束 |
...
MTP-163 把 MTP-162 的术语固定成 source layout contract。MTP-198 后且 Workbench cleanup 完成后，当前 forward-looking layout 只能落到 `Sources/DomainModel/`、`Sources/DataClient/<venue>/`、`Sources/DataEngine/`、`Sources/MessageBus/`、`Sources/Cache/`、`Sources/Database/`、`Sources/Trader/Strategies/EMA/`、`Sources/Trader/Coordination/`、`Sources/Trader/`、`Sources/Portfolio/`、`Sources/RiskEngine/`、`Sources/ExecutionEngine/`、`Sources/ExecutionClient/` 和 `Sources/Dashboard/`。`Sources/Workbench/` 只作为 historical / forbidden active path evidence 保留，不再是当前目标地形。

MTP-191 之后，MTP-163 中的 `Sources/Strategies/<strategy>/` 只作为 historical / compatibility / superseded layout anchor 保留；MTP-198 当时的 forward-looking concrete strategy canonical active path 只有 `Sources/Trader/Strategies/EMA/`。该 EMA-only 口径现在只作为 historical / audit evidence；Release v0.2.0 当前 active strategy scope 是 `EMA + RSI`，canonical path pattern 是 `Sources/Trader/Strategies/<strategy>/`。OrderBookImbalance、Momentum、MeanReversion 等非 EMA / RSI strategy 只能作为 future candidate，不进入当前 active release scope。

`MTP-163-DEPENDENCY-DIRECTION-CONTRACT`

MTP-163 固定依赖方向：`DataClient` 只依赖 `DomainModel`；`DataEngine` 通过 `DataClient` / `MessageBus` / `Cache` 处理 ingest 和 replay；`Trader/Strategies/<strategy>` 只能消费 domain / bus / cache / Portfolio / RiskEngine read-model inputs；`Trader` 可以协调 Strategies / Portfolio / RiskEngine / ExecutionEngine，但不得直连 `ExecutionClient`；`Workbench` 只能消费 ReadModel / ViewModel export。Release v0.2.0 当前 active strategy scope 只允许 EMA + RSI。
...
`DataClient/<venue>/` 是 exchange / venue scoped rule，一个交易所一个目录；MTP-163 的 `Strategies/<strategy>` 是历史 strategy scoped rule。MTP-191 之后，后续 strategy scoped rule 是 `Trader/Strategies/<strategy>`；MTP-198 当时只允许 `Trader/Strategies/EMA`。Release v0.2.0 当前示例表达 `DataClient/Binance/*` 和 active `Trader/Strategies/{EMA,RSI}/` 目录语义；RSI 必须由对应 v0.2.0 issue 单独落地，不授权 private stream runtime、signed/account endpoint、strategy runtime 或 trader process manager 绕过 gate。
...
`Trader Accounts / Coordination compatibility` 指 MTP-205 后的 Trader container 历史口径：`Trader = Accounts + Strategies/EMA + Coordination`。该 EMA-only 表述现在只作为 historical / audit evidence；Release v0.2.0 当前 active strategy scope 是 `EMA + RSI`。它只表达 source layout / docs / validation contract，不表示 Trader runtime、strategy scheduler、account session runtime、live coordinator、ExecutionClient、OMS、broker gateway 或 live command 已实现。

`MTP-205-TRADER-CONTAINER-AUTHORITATIVE-RELATIONSHIP`

MTP-205 contract Trader component set 是 `Sources/Trader/Accounts/`、`Sources/Trader/Strategies/EMA/` 和 `Sources/Trader/Coordination/RiskBinding/`。旧 `Trader = Accounts + Strategies + StrategyBindings + Coordination` 只能作为 historical evidence；Release v0.2.0 current / active / forward-looking wording 必须使用 `Trader = Accounts + Strategies/{EMA,RSI} + Coordination`，并保持 RSI 由对应 v0.2.0 issue 单独落地。
...
MTP-205 当时 active concrete strategy only `EMA`，canonical active path only `Sources/Trader/Strategies/EMA/`。该 EMA-only 口径现在只作为 historical / audit evidence；Release v0.2.0 当前 active strategy scope 是 `EMA + RSI`，非 EMA / RSI strategy 名称只能作为 future candidate、future-gated label、historical evidence 或 compatibility debt 出现。
...
MTP-208 后，`StrategyBindings` 只能作为 historical / compatibility / superseded term 保留；它不是 current active source path、Trader 下一级策略目录、concrete strategy implementation landing path 或 execution shortcut。

`MTP-208-TRADER-COORDINATION-RISKBINDING-CURRENT-LOCATION`

Current binding / adapter semantics 属于 `Sources/Trader/Coordination/RiskBinding/`。RiskBinding 是 local coordination adapter contract，不是 ExecutionClient、OMS、broker gateway、signed endpoint、account endpoint / listenKey、private stream runtime、Live PRO Console、trading button 或 live command。

`MTP-208-EMA-ONLY-ACTIVE-STRATEGY-DOC-GUARD`

Current active concrete strategy only `EMA`，canonical active path only `Sources/Trader/Strategies/EMA/`。非 EMA strategy、旧 `Sources/Strategies/<strategy>` 和 retained `StrategyBindings` references 只能作为 future candidate、historical evidence、compatibility debt 或 superseded context。

`MTP-208-NO-SOURCE-MOVE-PACKAGE-RUNTIME-GUARD`

MTP-208 是 docs wording retirement issue：不移动 source，不修改 `Package.swift`，不拆 SwiftPM target graph，不实现 Strategy runtime、Trader runtime、Live runtime、ExecutionClient、OMS、broker gateway、real order lifecycle、Live PRO Console、trading button、live command 或 order form。
...
`Sources/Strategies/` 不是 current active strategy source root、Package source root 或 exclude root。旧 `Sources/Strategies/<strategy>` 只能作为 historical / compatibility / superseded / migration-source language 保留；Release v0.2.0 current active concrete strategy scope 是 `EMA + RSI`，canonical path pattern 是 `Sources/Trader/Strategies/<strategy>/`。
...
MTP-211 stage closeout 指 `MTPRO Trader Accounts / Coordination Compatibility Consolidation v1` 的审计输入材料收口。它只汇总 MTP-205 至 MTP-210 的 contract、source boundary、validation wiring、wording retirement、Package cleanup 和 container completeness evidence，不等于最终 Stage Code Audit Report。

`MTP-211-STAGE-AUDIT-INPUT-MATERIAL`

MTP-211 的 stage audit input material 位于 `docs/audit/inputs/mtpro-trader-accounts-coordination-compatibility-consolidation-v1-stage-audit-input.md`。该材料服务 Parent Codex 后续 closure，不创建下一 Project / Issue，不推进 Todo，不授权 runtime、live、broker 或 L4 capability。

`MTP-211-NO-FINAL-STAGE-CODE-AUDIT`

MTP-211 不输出最终 Stage Code Audit Report。最终报告必须等待 MTP-205 至 MTP-211 全部 Done，并且 Linear Project status 被设置或确认为 `Completed/type=completed/completedAt` 后，由 Parent Codex 单独输出。

`MTP-211-TRADER-CONTAINER-COMPATIBILITY-CLOSEOUT`

MTP-211 后，Trader shared language 继续使用 `Trader = Accounts + Strategies/EMA + Coordination`：Accounts 是 identity / source / future gate；EMA 是唯一 current active concrete strategy；RiskBinding 是 local coordination adapter；旧 StrategyBindings 和 peer-level Sources/Strategies 只保留 historical / compatibility / superseded 语义。

`MTP-211-COMPATIBILITY-ENVELOPE-CLOSEOUT`

MTP-211 的 compatibility envelope closeout 指 `Core` 继续编译 Trader Accounts、Trader Strategies/EMA 和 Trader Coordination/RiskBinding source roots，但这只是 buildability envelope，不表示 SwiftPM target graph split、Trader runtime、Strategy runtime、Live runtime、ExecutionClient、OMS 或 broker gateway 已完成。

`MTP-211-FORBIDDEN-IMPLEMENTATION-AUDIT`

MTP-211 的 forbidden implementation audit 固定 no Strategy runtime、no Trader runtime、no Live runtime、no ExecutionClient、no OMS、no broker gateway、no signed/account/listenKey endpoint、no private WebSocket runtime、no real account read、no real order lifecycle、no Live PRO Console、no trading button、no live command、no order form、no Graphify、no Figma、no next Project / Issue 和 no next Todo promotion。

`MTP-211-STAGE-CLOSEOUT-VALIDATION`

MTP-211 validation 只证明 stage audit input material、validation matrix、automation readiness、compatibility envelope 和 forbidden implementation audit 已落仓；不证明任何 runtime、broker integration、private stream runtime 或 command-capable UI 已实现。
...
MTP-193 的 compatibility envelope 指 `Core` target 名称保持不变，但 EMA source root 改为 `Sources/Trader/Strategies/EMA/`；MTP-201 后 OrderBookImbalance 只保留为 `Sources/Core/Research/OrderBookImbalanceResearchEvidence.swift` historical research evidence。这不表示 SwiftPM target graph split 已完成。
...
`OrderBookImbalance research evidence retirement` 指 order-book imbalance research source 已从 MTP-194 的 Trader-owned non-EMA path `Sources/Trader/Strategies/OrderBookImbalance/` 退休到 `Sources/Core/Research/OrderBookImbalanceResearchEvidence.swift`。MTP-201 后该 evidence 只表达 historical research / parity / persistence support，不是当前 active concrete strategy；MTP-201 / MTP-198 的 EMA-only 事实现在只作为 historical / audit evidence。Release v0.2.0 当前 active strategy scope 是 `EMA + RSI`。该术语不改变 imbalance calculation、bias semantics、input source evidence、fixtures 或 `import Core` compatibility surface。
...
MTP-201 的 compatibility envelope 指 `Core` target 名称保持不变，并记录 OrderBookImbalance historical research evidence 位于 `Sources/Core/Research/OrderBookImbalanceResearchEvidence.swift`。这不表示 OrderBookImbalance 是当前 active concrete strategy，也不表示 SwiftPM target graph split 已完成。
...
MTP-201 validation language 必须证明 OrderBookImbalance active source root `Sources/Trader/Strategies/OrderBookImbalance/` 不存在，historical research evidence 位于 `Sources/Core/Research/OrderBookImbalanceResearchEvidence.swift`，`Package.swift` 不包含 `"Trader/Strategies/OrderBookImbalance"` 或 `"Strategies/OrderBookImbalance"`，focused OrderBookImbalance tests 和完整 checks 仍通过。

`MTP-195-STRATEGYBINDINGS-BINDING-PROTOCOL-ADAPTER-CONTRACT`

`StrategyBindings binding protocol / coordination adapter contract` 指 MTP-195 时 `Sources/Trader/StrategyBindings/` 保存的 strategy instance 与 Trader context / RiskEngine / Portfolio evidence 通用 binding protocol、adapter contract 和 deterministic local coordination evidence。MTP-202 后该 active source root 已迁入 `Sources/Trader/Coordination/RiskBinding/`。

`MTP-195-CONCRETE-STRATEGY-NON-LANDING-GUARD`

MTP-202 后 `Sources/Trader/Coordination/RiskBinding/` 不得作为 EMA、OrderBookImbalance 或未来具体策略的源码落点。MTP-201 后 EMA 当前 active root 是 `Sources/Trader/Strategies/EMA/`，OrderBookImbalance 只保留为 `Sources/Core/Research/OrderBookImbalanceResearchEvidence.swift` historical research evidence；具体 strategy lifecycle、signals、proposal implementation、quoter、hedger 和 strategy-specific business rules 必须继续落在 `Sources/Trader/Strategies/<strategy>/`，但非 EMA 不得被写成当前 active concrete strategy。

`MTP-195-STRATEGYBINDINGS-COMPATIBILITY-ENVELOPE`

MTP-195 的 compatibility envelope 指 `Core` target 名称保持不变。MTP-202 后 `Core` target 改为编译 `Sources/Trader/Coordination/RiskBinding/PaperActionRiskLink.swift`，继续保持 import surface 和 tests buildability，不表示 SwiftPM target graph split、Trader runtime、strategy scheduler 或 live coordinator 已实现。

`MTP-195-NO-DIRECT-EXECUTION-BROKER-OMS-LIVE-GUARD`

RiskBinding 不得引入 `RiskBinding -> ExecutionClient`、`RiskBinding -> broker command`、`RiskBinding -> OMS command`、`RiskBinding -> signed/account endpoint`、`RiskBinding -> private stream runtime`、`RiskBinding -> Live PRO Console`、`RiskBinding -> trading button`、`RiskBinding -> live command` 或 `RiskBinding -> order form` 路径。`PaperActionRiskLink` 仍只表达 paper proposal -> risk query / blocker evidence 的本地只读链路。

`MTP-195-STRATEGYBINDINGS-BOUNDARY-VALIDATION`

MTP-202 validation language 必须证明 `TraderCoordinationRiskBindingBoundaryEvidence`、focused XCTest、docs anchors 和 automation readiness checks 一致覆盖：RiskBinding 只是 generic binding protocol / coordination adapter，concrete strategies remain Trader-owned，且无 direct execution / broker / OMS / live command path。

`MTP-196-TRADER-OWNED-STRATEGY-PATH-VALIDATION`

`Trader-owned strategy path validation` 指本地 deterministic checks 直接验证 EMA 的当前 active implementation files 位于 `Sources/Trader/Strategies/EMA/`，并验证 OrderBookImbalance active source root 已退休到 Core research evidence；MTP-201 后不得把 OrderBookImbalance 写成当前 active concrete strategy。该验证同时覆盖 `Package.swift` 不能回流到旧 `"Strategies/EMA"`、`"Strategies/OrderBookImbalance"` 或 `"Trader/Strategies/OrderBookImbalance"` source roots。
...
MTP-202 validation 必须继续使用 `TraderCoordinationRiskBindingBoundaryEvidence` 证明 `Sources/Trader/Coordination/RiskBinding/` 只是 generic binding protocol / coordination adapter contract，不是 concrete strategy source root，并证明旧 `Sources/Trader/StrategyBindings/` 不再是 active source root。
...
MTP-201 只执行 non-EMA active source retirement、local validation evidence、docs anchors 和 automation readiness checks，不拆 SwiftPM target graph，不实现 Strategy runtime、Trader runtime、ExecutionClient、OMS、broker gateway、signed/account endpoint、private stream runtime、Live PRO Console、trading button、live command 或 order form。

`MTP-198-EMA-ONLY-TRADER-STRATEGY-LAYOUT-CONTRACT`

`EMA-only Trader strategy layout` 指 MTP-198 当时 active concrete strategy 只有 `EMA`，唯一 canonical active concrete strategy path 是 `Sources/Trader/Strategies/EMA/`。该术语现在只表达 historical / audit evidence，不再作为 Release v0.2.0 当前边界；Release v0.2.0 当前 active strategy scope 是 `EMA + RSI`。
...
`RSI`、`OrderBookImbalance`、`Momentum` 和 `MeanReversion` 只能作为 future strategy candidate / future-gated strategy label。MTP-201 后 `Sources/Trader/Strategies/OrderBookImbalance/` 已退休，不是当前 active strategy；保留的 OrderBookImbalance 类型只作为 `Sources/Core/Research/OrderBookImbalanceResearchEvidence.swift` historical research evidence。

`MTP-198-STRATEGYBINDINGS-NOT-FIRST-LEVEL-STRATEGY-DIRECTORY`

`StrategyBindings not first-level strategy directory` 指 MTP-202 后 `Sources/Trader/StrategyBindings/` 不再是 current active source root；当前 binding / adapter source root 是 `Sources/Trader/Coordination/RiskBinding/`。RiskBinding 也不是 concrete strategy implementation root，不能承载 EMA、RSI、OrderBookImbalance、Momentum、MeanReversion 或未来具体 strategy 的 lifecycle、signals、proposal implementation、quoter、hedger 或 strategy-specific business rules。

`MTP-198-TRADER-COORDINATION-BINDING-RESPONSIBILITY`

`Trader coordination binding responsibility` 指 binding / adapter semantics 归 `Sources/Trader/Coordination/` 管理。当需要表达 strategy instance 与 account context、RiskEngine、Portfolio evidence 或 ExecutionEngine evidence 的连接关系时，应使用 `Trader/Coordination/<binding>/` boundary；MTP-202 当前使用 `Sources/Trader/Coordination/RiskBinding/`，而不是把 binding 当作 first-level strategy directory。

`MTP-198-FORBIDDEN-STRATEGY-PATH-EXECUTION-BYPASS-TAXONOMY`

MTP-198 forbidden strategy bypass taxonomy 固定 no direct Strategy / RiskBinding to ExecutionClient、broker command、OMS、executable order command、signed/account endpoint、private stream runtime、Live PRO Console、trading button、live command 或 order form。Paper proposal、signal、risk decision 和 portfolio projection 不能升级为真实交易命令。

`MTP-198-NO-SOURCE-MOVE-PACKAGE-RUNTIME-GUARD`

MTP-198 是 contract / docs boundary，不移动 `Sources`，不删除 OrderBookImbalance compatibility debt，不移动 StrategyBindings，不修改 `Package.swift`；MTP-202 后该 move 已由 authorized issue 执行到 `Sources/Trader/Coordination/RiskBinding/`。当前仍不新增 SwiftPM target，不实现 Strategy runtime、Trader runtime、ExecutionClient、OMS、broker adapter、signed/account/listenKey endpoint、private stream runtime、Live PRO Console、trading button、live command 或 order form。

`MTP-198-EMA-ONLY-LAYOUT-VALIDATION`

MTP-202 validation language 必须证明 active concrete strategy only EMA、canonical active path only `Sources/Trader/Strategies/EMA/`、non-EMA future candidates only、old StrategyBindings no longer first-level active source directory、binding semantics under `Sources/Trader/Coordination/RiskBinding/`，以及 no runtime guard。
...
`Sources/Dashboard/FutureLiveProConsole/` 目前只是 future boundary label，不是当前目录、target、route、ViewModel 或 runtime implementation。该 label 只说明后续 L4 规划可能需要独立产品面，不授权当前创建 Live PRO Console source tree。
...
`Strategies / Trader / Portfolio physical migration` 指 MTP-187 把 EMA strategy lifecycle、strategy signal、paper proposal 和 order-book research strategy 从旧 `Sources/Core/` 迁入当时的 `Sources/Strategies/EMA/` 与 `Sources/Strategies/OrderBookImbalance/`，把 proposal-to-risk binding 迁入 `Sources/Trader/StrategyBindings/`，并把 paper account / portfolio projection、portfolio projection update 和 simulated exchange portfolio projection parity 迁入 `Sources/Portfolio/`。MTP-193 后 EMA current active source path 是 `Sources/Trader/Strategies/EMA/`；MTP-194 后 OrderBookImbalance source placement 只作为 historical / compatibility evidence；MTP-198 后当前 active concrete strategy 只有 EMA。MTP-187 只做 physical source migration 和兼容 target 配置，不新增真实 Strategies / Trader / Portfolio SwiftPM target。

MTP-191 后续把 MTP-187 的 strategy physical locations 标记为 compatibility / superseded path；MTP-193 已把 EMA concrete strategy files 移到 `Sources/Trader/Strategies/EMA/`，MTP-201 已把 OrderBookImbalance 从 active Trader strategy root 退休到 `Sources/Core/Research/OrderBookImbalanceResearchEvidence.swift`。

`MTP-187-STRATEGIES-COMPATIBILITY-ENVELOPE`

MTP-187 的 `Strategies compatibility envelope` 指现有 `Core` target 继续编译 historical strategy / research source roots。MTP-201 后该 envelope 只能把 `Sources/Trader/Strategies/EMA/` 作为当前 active EMA path，把 `Sources/Core/Research/OrderBookImbalanceResearchEvidence.swift` 作为 historical research evidence。它只保持 buildability，不授权非 EMA active strategy、Strategy runtime、scheduler、live quoter / hedger、direct Strategy -> ExecutionClient path、broker command 或 executable order command。
...
MTP-189 的 `App compatibility envelope` 指现有 `App` target 继续编译 `Sources/Dashboard/ReadModels/`、`Sources/Dashboard/Report/`、`Sources/Dashboard/`、`Sources/Dashboard/Events/`、`Sources/Dashboard/FutureLiveProConsole/` 和 `Sources/Dashboard/DashboardShell.swift`。这只保持 buildability，不授权 Workbench runtime、Dashboard runtime inspector、broker connect UI、account connect UI 或 final target graph split。
...
MTP-189 后 `Sources/Dashboard/FutureLiveProConsole/` 只是 future-gated boundary label 的 physical location。它不是 current Live PRO Console implementation，不包含 command controls、emergency stop、shutdown、restore、ExecutionClient request UI、OMS command UI 或 production operations command。
...
`Trader-owned strategies target boundary` 指后续多个 concrete strategies 都放在 `Sources/Trader/Strategies/<strategy>/`，由 `TraderStrategies` target 管理。MTP-216 当时 active concrete strategy only `EMA`，canonical active path only `Sources/Trader/Strategies/EMA/`；Release v0.2.0 当前 active strategy scope 是 `EMA + RSI`。策略只提出 signal / paper-neutral proposal evidence，不直连 ExecutionClient、broker、OMS、Workbench、Dashboard 或 UI command surface。
...
MTP-218 data target split 指 `Package.swift` 已新增 `DataClient`、`DataEngine` 和 `Cache` library products / targets，并用 `Sources/TargetGraph/*` boundary types 证明三个 data-layer targets 可以被 SwiftPM 解析和测试。该 evidence 不表示旧 `Core`、`Adapters` 或 `Runtime` compatibility envelope 已退休。

`MTP-218-DATACLIENT-TARGET-SPLIT`

`DataClient target boundary` 指只依赖 `DomainModel` 的 public read-only venue data input target；当前 compiled boundary root 是 `Sources/TargetGraph/DataClient/`。既有 Binance public market data implementation 仍通过 `Adapters` compatibility envelope 使用，不升级成 signed endpoint、account endpoint、listenKey、private stream runtime 或 broker adapter。

`MTP-218-CACHE-TARGET-SPLIT`

`Cache target boundary` 指只依赖 `DomainModel` 和 `MessageBus` 的 read-model state surface；当前 compiled boundary root 是 `Sources/TargetGraph/Cache/`。Cache 可以表达 runtime-derived state semantics，但不能拥有 durable facts、Database schema、broker state、account payload 或 UI command surface。

`MTP-218-DATAENGINE-TARGET-SPLIT`

`DataEngine target boundary` 指依赖 `DomainModel`、`DataClient`、`MessageBus` 和 `Cache` 的 ingest / replay / quality target；当前 compiled boundary root 是 `Sources/TargetGraph/DataEngine/`。既有 ingest / scenario replay / data quality implementation 仍通过 `Core` / `Runtime` compatibility envelope 使用，不表示 streaming runtime、private stream、account endpoint 或 broker route 已实现。

`MTP-218-DATACLIENT-DATAENGINE-CACHE-DEPENDENCY-DIRECTION`

MTP-218 的 dependency direction 是 `DataClient -> DomainModel`、`Cache -> DomainModel / MessageBus`、`DataEngine -> DomainModel / DataClient / MessageBus / Cache`。禁止 data targets 依赖 Trader、TraderStrategies、RiskEngine、ExecutionEngine、ExecutionClient、Workbench、Dashboard、broker、OMS、signed endpoint、account endpoint、listenKey、private stream runtime、Database schema 或 broker state。

`MTP-218-PUBLIC-READ-ONLY-DATA-BOUNDARY`

`Public read-only data boundary` 指 DataClient 只获取公开 market data，不能读取真实 account / position / balance，不能创建 listenKey，不能调用 signed/account endpoint，不能连接 broker 或 execution adapter。

`MTP-218-READMODEL-STATE-SURFACE`

`Read-model state surface` 指 Cache 只保留可由 facts / replay 重建的本地 state view，不保存 authoritative facts，不暴露 persistence schema，不作为 broker state、account payload 或 UI command source。

`MTP-218-DATA-COMPATIBILITY-ENVELOPE-RETAINED`

MTP-218 保留 `Adapters` / `Core` / `Runtime` compatibility envelope，表示既有 implementation 暂不搬迁，以保持当前行为不变；MTP-222 才能退休已被 split 取代的 obsolete envelope 或 stale target anchors。

`MTP-218-TARGETGRAPH-TEST-EVIDENCE`

`TargetGraphTests` 在 MTP-218 继续直接 import `DataClient`、`DataEngine` 和 `Cache`，验证 allowed dependencies、forbidden dependencies、compatibility envelope 和 no signed / account / listenKey / broker / runtime capability flags。

`MTP-218-NO-SIGNED-ACCOUNT-BROKER-GUARD`

MTP-218 不授权 Strategy runtime、Trader runtime、Live runtime、ExecutionClient implementation、OMS、broker gateway、signed endpoint、account endpoint / listenKey、private WebSocket runtime、account snapshot runtime、real account read、real order lifecycle、Live PRO Console、trading button、live command、order form 或 L4 capability。

`MTP-219-TRADER-PORTFOLIO-RISK-TARGET-SPLIT-EVIDENCE`

MTP-219 Trader / Portfolio / Risk target split 指 `Package.swift` 已新增 `TraderStrategies`、`Trader`、`Portfolio` 和 `RiskEngine` library products / targets，并用 `Sources/TargetGraph/*` boundary types 证明 coordination / financial state / pre-execution risk targets 可以被 SwiftPM 解析和测试。该 evidence 不表示旧 `Core` compatibility envelope 已退休。

`MTP-219-TRADERSTRATEGIES-TARGET-SPLIT`

`TraderStrategies target boundary` 指只承载 Trader-owned concrete strategy definitions 的 target。MTP-219 当时 active concrete strategy only `EMA`，canonical active source root only `Sources/Trader/Strategies/EMA/`；Release v0.2.0 当前 active strategy scope 是 `EMA + RSI`。非 EMA / RSI strategy 只能作为 future candidate / historical evidence / compatibility debt，不得成为 active release source。

`MTP-219-TRADER-TARGET-SPLIT`

`Trader target boundary` 指 `Accounts + Strategies/EMA + Coordination` container 的 target。它可以消费 `TraderStrategies`、`Portfolio`、`RiskEngine` 和 MTP-220 后已解析的 `ExecutionEngine` evidence/context，但不能直连 `ExecutionClient`、broker、OMS、signed/account endpoint、listenKey 或 UI command surface。MTP-219 里的 `Trader -> ExecutionEngine` deferred wording 只作为 historical before-MTP-220 evidence 保留。

`MTP-219-PORTFOLIO-TARGET-SPLIT`

`Portfolio target boundary` 指独立 financial state projection target，负责 positions、net positions、cash / equity、PnL、margin、open value 和 paper projection semantics。Portfolio 不拥有 Trader account identity，不读取 broker account state、account endpoint payload、signed endpoint、listenKey 或 private stream runtime。

`MTP-219-RISKENGINE-TARGET-SPLIT`

`RiskEngine target boundary` 指 pre-execution risk target，消费 Portfolio / Cache / MessageBus evidence 并输出 paper risk blocker / allowed / blocked evidence。RiskEngine 不是 live risk runtime、broker gateway、ExecutionClient wrapper、OMS command path 或 executable order command source。

`MTP-219-TRADER-PORTFOLIO-RISK-DEPENDENCY-DIRECTION`

MTP-219 的 historical dependency direction 是 `Portfolio -> DomainModel / MessageBus / Cache / Database`、`RiskEngine -> DomainModel / MessageBus / Cache / Portfolio`、`TraderStrategies -> DomainModel / MessageBus / Cache / Portfolio / RiskEngine`、`Trader -> DomainModel / MessageBus / Cache / TraderStrategies / Portfolio / RiskEngine`，并把 `Trader -> ExecutionEngine` 延后到 MTP-220。MTP-222 当前 active target graph 已包含 `Trader -> ExecutionEngine`。

`MTP-219-EMA-ONLY-ACTIVE-STRATEGY-BOUNDARY`

`EMA-only active strategy boundary` 指当前唯一 active concrete strategy 是 `EMA`。`RSI`、`OrderBookImbalance`、`Momentum`、`MeanReversion` 或其他 strategy 不得作为 active source root、Package source root、test root 或 target source root 回流。

`MTP-219-TRADER-CONTAINER-ACCOUNTS-EMA-COORDINATION`

`Trader container` 在 MTP-219 固定为 `Accounts + Strategies/EMA + Coordination`：account context root 是 `Sources/Trader/Accounts/`，active strategy root 是 `Sources/Trader/Strategies/EMA/`，coordination root 是 `Sources/Trader/Coordination/RiskBinding/`。旧 `Sources/Trader/StrategyBindings/` 和 peer-level `Sources/Strategies/` 只能作为 historical / compatibility / superseded context。
...
MTP-221 Workbench / Dashboard target split 是历史 split evidence。后续 cleanup 已退休 `Workbench` product / target、`Sources/Workbench/`、`App` product / target 和 `Sources/AppCompatibility`。当前 active UI target 只有 `Dashboard` executable，它直接拥有 read-model-only shell、ReadModels、Report、Events 和 FutureLiveProConsole future label，并只消费 `Core` / `Persistence` 导出的 read model、ViewModel 和 projection snapshot。

`MTP-221-WORKBENCH-TARGET-RETIRED`

`Workbench target boundary` 当前已退休。`Package.swift` 不再包含 `Workbench` product / target，`Sources/Workbench/` 和 `Sources/Workbench/TargetGraph/WorkbenchTargetBoundary.swift` 只能作为 historical / forbidden active path 证据引用。

`MTP-221-DASHBOARD-TARGET-SPLIT`

`Dashboard target boundary` 指 macOS shell / smoke executable display surface。它可以直接依赖 `Core` / `Persistence` 的 read-model-only exports，装载 `DashboardViewModel` 和 `DashboardShellSnapshot`；不能依赖 `Adapters`、`Runtime`、`ExecutionClient`、broker、OMS、schema、account payload 或 live command。

`MTP-221-DASHBOARD-READ-MODEL-DEPENDENCY-DIRECTION`

MTP-221 后续 cleanup 的 dependency direction 当前收口为 `Dashboard -> Core / Persistence read-model and ViewModel exports only`。`App` product / target、`Sources/AppCompatibility`、`Workbench` product / target 和 `Sources/Workbench/` 已退休；既有 `App` / `Workbench` import surface 不再作为 active compatibility envelope，`Tests/AppTests` 直接 import `Dashboard`。

`MTP-221-READ-MODEL-VIEWMODEL-ONLY`

Dashboard 的 read-model-only 语义表示它只能展示已经存在的 evidence surface 和 ViewModel snapshot；不得把 Report、Events、Live Monitoring、Strategy readiness、Account Position Balance 或 Private Stream Simulation Gate evidence 升级成 runtime connection、adapter call、schema access、broker state、account payload、live command、trading button 或 order form。
...
MTP-222 compatibility anchor retirement 指把旧 compatibility envelope / stale target anchors 从 active wording 中退休。MTP-217 至 MTP-221 已建立当前可编译 target graph；MTP-216 的 `Core / Adapters / Persistence / Runtime / App / Dashboard` snapshot 只作为 historical before-state evidence 保留。

`MTP-222-CURRENT-TARGET-GRAPH-SNAPSHOT`

Current target graph 指 `DomainModel`、`MessageBus`、`Database`、`DataClient`、`Cache`、`DataEngine`、`Portfolio`、`RiskEngine`、`ExecutionClient`、`ExecutionEngine`、`TraderStrategies`、`Trader` 和 `Dashboard`。Current UI dependency direction 是 `Dashboard -> Core / Persistence read-model exports only`；`App` 和 `Workbench` compatibility exports 均已退休。

`MTP-222-HISTORICAL-COMPATIBILITY-EVIDENCE-RETAINED`

Historical compatibility evidence 指旧 `Core`、`Adapters`、`Persistence`、`Runtime`、`App` compatibility targets、旧 `Sources/Strategies/<strategy>`、旧 `Sources/Trader/StrategyBindings/`、旧 `Dashboard -> App` 和旧 `App -> Core, Persistence` references 可以继续存在，但必须带 historical / compatibility / superseded / before-state 语义，不能表达 current active graph、current active strategy path 或 current UI dependency。

`MTP-222-STALE-ACTIVE-ANCHOR-RETIREMENT`

Stale active anchor retirement 表示 active source anchors 必须指向 `Sources/Trader/Strategies/EMA/`、`Sources/Trader/Coordination/RiskBinding/`、`TraderStrategies`、`Trader` 和 `Dashboard -> Core / Persistence`。旧 peer-level `Sources/Strategies/`、旧 first-level `Sources/Trader/StrategyBindings/` 和旧 `Sources/Workbench/` 不得回流为 active source root、Package root、test root 或 target name。
...
`Real module source root` 指后续 active target path 的真实目标目录，例如 `Sources/DomainModel/`、`Sources/MessageBus/`、`Sources/DataClient/`、`Sources/DataEngine/`、`Sources/Trader/Strategies/<strategy>/`、`Sources/Trader/Accounts/`、`Sources/Trader/Coordination/`、`Sources/ExecutionEngine/`、`Sources/ExecutionClient/` 和 `Sources/Dashboard/`。Release v0.2.0 当前 active strategy scope 是 `EMA + RSI`；非 EMA / RSI strategy 仍只能作为 future candidate。`Sources/Workbench/` 当前已退役，只能作为 historical / forbidden active path evidence。
...
L4 live production command contract 指 GH-452 至 GH-472 的共同顶层合同。它只定义 read-only evidence 如何在后续 gate 全部满足后进入 guarded command，不表示当前已经具备 production trading。

`GH-452-READONLY-TO-GUARDED-COMMAND-RULE`

Read-only -> guarded command 是一个受门禁保护的转换规则：
...
该规则不允许 Strategy、Trader、Dashboard 或 MessageBus 直接绕过 RiskEngine / OMS / ExecutionClient gate，也不允许 Dashboard 直连 broker。

`GH-452-NO-DEFAULT-REAL-TRADING-POLICY`

No-default-real-trading policy 表示 production trading 默认关闭。#471 之前不得通过环境变量、配置、UI 或 hidden flag 默认启用 production endpoint、真实 broker、真实 submit / cancel / replace、execution report、broker fill 或 reconciliation。
...
L4 signed endpoint private stream boundary 指 signed read-only、private stream 和 command runtime 的分层边界。它依赖 GH-452 command contract 和 GH-453 credential environment gate，但不实现 endpoint client、listenKey、private WebSocket、account read 或 command runtime。

`GH-454-SIGNED-REQUEST-CAPABILITY-TAXONOMY`

Signed request capability taxonomy 只保留 credential reference、timestamp identity、recvWindow identity、API-key header identity、request signature identity、signed account read-only、listenKey lifecycle、private event source 和 command runtime boundary 等语义身份。它不是 request builder，不生成 HMAC，不构造 header，不调用 signed endpoint。

`GH-454-LISTENKEY-PRIVATE-WEBSOCKET-FUTURE-CONTRACT`

listenKey / private WebSocket future contract 必须把 create、keep-alive、open、event source identity 和 close 拆成独立 gate。GH-454 不创建 listenKey，不 keep-alive，不 close，不打开 WebSocket，不 reconnect，也不消费真实 private event。

`GH-454-ACCOUNT-SNAPSHOT-PRIVATE-EVENT-SOURCE-IDENTITY`

Account snapshot / private event source identity 只能描述未来 evidence 来源，例如 signed account snapshot、balance update、position update、margin state、execution report 和 listenKey session。它不能包含 real account payload、listenKey value、signed request payload、broker payload、execution report payload 或订单命令。
...
L4 private stream account snapshot read-only runtime 指 disabled-by-default、sandbox / local fixture-first 的 private stream source / freshness evidence gate。它依赖 GH-455 canonical signed account evidence，但不创建 listenKey、不打开 private WebSocket、不消费真实 private event。

`GH-456-PRIVATE-STREAM-SOURCE-IDENTITY`

Private stream source identity 只描述 signed account snapshot source、private balance update source、private position update source、private margin update source 和 listenKey session source identity。listenKey session source identity 是 evidence label，不是 listenKey value、WebSocket session 或 broker session。

`GH-456-ACCOUNT-SNAPSHOT-READ-MODEL-UPDATE`

Account snapshot read-model update 表示 private stream / account snapshot evidence 只写入 canonical read model。它不包含 raw private payload、account endpoint JSON、broker state、execution report payload、real balance、real position、margin / leverage runtime 或 real PnL。

`GH-456-FRESHNESS-STALE-BLOCKED-MISSING-DISCONNECT-EVIDENCE`

Freshness evidence 必须覆盖 fresh、stale、blocked、missing 和 disconnected。stale / blocked / missing / disconnected 只作为 read-model status，不触发 reconnect、listenKey keep-alive、broker fallback、order retry 或 trading command。
...
L4 ExecutionEngine -> ExecutionClient sandbox path 指 RiskEngine-approved command proposal 进入 ExecutionEngine 后，由 ExecutionEngine 通过 GH-459 sandbox adapter 生成 request / response evidence，并关联 GH-462 local transition evidence 的本地 deterministic evidence path。它不是 production execution runtime。

`GH-463-RISKENGINE-APPROVED-COMMAND-PROPOSAL`

RiskEngine-approved command proposal 指 pre-execution boundary 已放行、且只能从 ExecutionEngine handoff 进入 sandbox ExecutionClient 的 proposal。direct Trader、direct Strategy 和 Live PRO Console command 都不能成为 proposal source。

`GH-463-SANDBOX-EXECUTIONCLIENT-HANDOFF`

Sandbox ExecutionClient handoff 指 ExecutionEngine 调用 GH-459 sandbox ExecutionClient adapter 的受控输出路径。该 handoff 不生成 signed request，不读取 secret，不连接 production venue，不触碰 broker gateway。

`GH-463-COMMAND-RESPONSE-EVENT-EVIDENCE`

Command / response / event evidence 必须覆盖 submit、cancel、replace command evidence、sandbox response evidence，以及 proposal accepted、request dispatched、response recorded、local transition evidence linked 四类 event。Event 只作为 deterministic audit evidence，不写 Portfolio，也不执行 reconciliation。

`GH-463-NO-DIRECT-TRADER-STRATEGY-EXECUTIONCLIENT`

No direct Trader / Strategy -> ExecutionClient 固定 Trader、Strategy、Live PRO Console 都不能直接调用 ExecutionClient。ExecutionEngine handoff 必须保留 RiskEngine boundary 和 OMS local transition evidence。
...
L4 kill switch / incident shutdown gate 指本地 deterministic shutdown evidence：它消费 GH-464 incident stop decision，输出 source identity、submit / cancel / replace shutdown decision 和 recovery boundary。该 gate 不是 production operations runbook。

`GH-465-INCIDENT-STOP-SOURCE-IDENTITY`

Incident stop source identity 必须记录 source kind、source id、incident id、upstream risk decision id 和 reason，并绑定 GH-464 incident stop outcome。source identity 不能授权 auto recovery，不能触碰 broker gateway、production operations runtime 或 Live command surface。

`GH-465-SUBMIT-CANCEL-REPLACE-SHUTDOWN-RULES`

Submit / cancel / replace shutdown rules 固定 incident stop active 后三类 command 全部被 command shutdown gate 阻断。Decision 只输出 blocked evidence，不执行 command、不调用 ExecutionClient、不提交 / 撤销 / 替换真实订单。

`GH-465-DASHBOARD-AUDIT-SHUTDOWN-EVIDENCE`

Dashboard / audit shutdown evidence 指 shutdown gate 可以被 Dashboard / audit 解释为 read-model / evidence record，但不能创建 Live PRO Console、order form、trading button、emergency stop button 或 production command。

`GH-465-NO-AUTOMATIC-RECOVERY`

No automatic recovery 固定 shutdown 后恢复必须等待 manual review evidence、fresh RiskEngine gate 和 future production cutover gate。GH-465 不实现 restore command，也不允许自动绕过 shutdown gate。
...
L4 sandbox validation matrix closeout 指 GH-452 至 GH-469 的 issue-level evidence 汇总收口。它只证明 sandbox validation matrix 完整，不定义 production cutover，不输出最终 Stage Code Audit Report。

`GH-470-READ-RISK-EXECUTION-OMS-RECONCILIATION-AUDIT-UI-GATE`

Read / risk / execution / OMS / reconciliation / audit / UI gate closeout 固定 L4 sandbox evidence chain 已覆盖：signed account / private stream read-only evidence、APB read model、RiskEngine pre-trade gate、ExecutionClient sandbox path、OMS lifecycle、OMS / broker / portfolio reconciliation、audit trail / incident replay、Dashboard / Live PRO Console split 和 guarded UI surface。

`GH-470-NO-DEFAULT-PRODUCTION-TRADING`

No default production trading 固定 GH-470 后仍没有 production command、production endpoint enablement、real broker gateway、real submit / cancel / replace、order form 或 trading button。

`GH-470-NO-SECRET-RAW-BROKER-PAYLOAD`

No secret / raw broker payload 固定 GH-470 closeout 不读取 API key / secret，不存储 secret，不调用 signed endpoint，不创建 listenKey，不打开 private WebSocket，不把 raw broker payload 送入 Dashboard，不消费 production broker report。
...
L4 Stage Audit input 指 `MTPRO L4 Live Production / Trading Commands v1` 的 Project closure 前输入材料。它汇总 GH-452 至 GH-471 的 issue / PR / merge evidence、validation matrix、forbidden capability evidence 和 Root Docs Delta input，但不是最终 Stage Code Audit Report，也不是 production approval。

`GH-472-EVIDENCE-CHAIN-TRACE`

Evidence chain trace 固定 GH-452 至 GH-471 每个 issue 都必须有对应 merged PR、required check evidence、merge commit 和 closed / done issue state，供后续 final Stage Code Audit Report 复核。

`GH-472-COMMAND-RISK-EXECUTION-AUDIT-UI-GATE-TRACE`

Command / risk / execution / audit / UI gate trace 指 command contract、credential / signed boundary、read-only account / stream evidence、ExecutionClient / ExecutionEngine sandbox path、OMS lifecycle、RiskEngine pre-trade gate、kill switch、reconciliation、audit trail、Dashboard / Live PRO Console split、guarded UI、sandbox validation matrix 和 production cutover future gate 的可追溯链。

`GH-472-NO-DEFAULT-PRODUCTION-TRADING`

No default production trading 在 GH-472 后仍保持 production cutover execution、production endpoint、secret read / storage、signed endpoint、broker gateway、production command、order form、trading button 和 real submit / cancel / replace 全部关闭。

`GH-472-NO-NEXT-PROJECT-AUTO-PROMOTION`

No next Project auto promotion 固定 GH-472 完成后只能进入 Human 已要求的 Project closure flow；不得自动创建下一 Project / Issue，不得推进下一 Todo，不得写 future direction。

`TVM-L4-STAGE-AUDIT-INPUT-CLOSEOUT`

L4 stage audit input closeout matrix 必须证明 stage audit input material、evidence chain trace、gate trace、forbidden capability evidence、Root Docs Delta input 和 no-next-project stop rule 全部成立。

## GH-563 Release v0.2.0 Binance Spot + Perp EMA/RSI Contract Terms

`GH-563-RELEASE-V020-BINANCE-SPOT-PERP-EMA-RSI-CONTRACT`

Release v0.2.0 Binance Spot + USDⓈ-M Perpetual + EMA/RSI contract 指 GitHub fallback queue GH-563 至 GH-596 的共同顶层合同。它只授权在对应 issue scope 内逐步建设 Binance Spot / Binance USDⓈ-M Perpetual、EMA / RSI、NTPRO scoped alignment、dry-run / testnet-first release evidence 和 no-default-production-trading evidence，不授权默认 production trading。

`GH-563-BINANCE-SPOT-PERP-ACTIVE-SCOPE`

Binance Spot + Perp active scope 指 release v0.2.0 的唯一 active venue 是 Binance，唯一 active product types 是 Spot 和 USDⓈ-M Perpetual。非 Binance venue、非 Spot product 和非 USDⓈ-M Perpetual product 只能作为 future / forbidden / non-active scope 记录，不能写成 release v0.2.0 当前 active capability。

`GH-563-EMA-RSI-ACTIVE-STRATEGY-SCOPE`

EMA / RSI active strategy scope 指 release v0.2.0 的唯一 active concrete strategies 是 EMA 和 RSI。两者必须保持 Trader-owned strategy layout、paper/live-neutral proposal isolation 和 CommandGateway / RiskEngine / ExecutionEngine / OMS / Event Store gate，不得直连 ExecutionClient、broker、OMS 或 Dashboard live command。

`GH-563-NTPRO-SCOPED-ALIGNMENT-MATRIX`

NTPRO scoped 100% alignment 指 release v0.2.0 只在 Binance-only、Spot + USDⓈ-M Perpetual-only、EMA + RSI-only、CommandGateway / RiskEngine / ExecutionEngine / OMS / Event Store gate、validation evidence 和 no-default-production-trading 口径上对齐。它不表示复制 NTPRO 代码，不表示引入 NTPRO runtime dependency，不表示自动扩大 MTPRO release scope。

`GH-563-ACCEPTANCE-MATRIX`

Release v0.2.0 acceptance matrix 必须覆盖 GH-563 至 GH-596：contract、ownership gap、target smoke、Spot / Perp market data、signed account read、private stream、read-model mapping、Trader lifecycle、EMA / RSI runtime、strategy coordination、RiskEngine、CommandGateway、ExecutionEngine / OMS、Spot / Perp ExecutionClient testnet SCR、execution report / broker fill parser、reconciliation、Dashboard monitoring、controlled command surface、kill switch、dry-run / testnet validation、no-default-production-trading guard、NTPRO scoped alignment validation、operator runbook、validation matrix closeout、Stage Code Audit input、Root Docs Refresh input 和 final release closure。

`GH-563-NO-DEFAULT-PRODUCTION-TRADING`

No-default-production-trading 在 release v0.2.0 中表示 production trading、production endpoint、production secret read、production submit / cancel / replace、production OMS、production Dashboard command surface、non-Binance venue、non-Spot / non-USDS-M product 和 non-EMA / non-RSI active strategy 全部默认关闭。

`TVM-RELEASE-V020-BINANCE-SPOT-PERP-EMA-RSI-NTPRO-ALIGNMENT`

Release v0.2.0 Binance Spot + Perp EMA/RSI NTPRO alignment matrix 必须证明 release contract、active venue / product / strategy scope、NTPRO scoped alignment、acceptance matrix 和 no-default-production-trading boundary 全部成立，并为 GH-564 至 GH-596 的逐 issue evidence 回填提供稳定锚点。

## GH-564 Release v0.2.0 Root Docs Boundary Refresh Terms

`GH-564-RELEASE-V020-ROOT-DOCS-BOUNDARY-REFRESH`

Release v0.2.0 root docs boundary refresh 指 `README.md`、`architecture.md`、`docs/roadmap.md`、`docs/validation/latest-verification-summary.md` 和本文档必须使用同一个当前口径描述 release v0.2.0：activeVenue == Binance，activeProductTypes == [spot, usdsPerpetual]，activeStrategies == [ema, rsi]，productionTradingEnabledByDefault == false。

`GH-564-PRODUCTION-CAPABILITY-GATED-NOT-MISSING`

Production capability gated not missing 指生产能力不是缺失口径，也不是默认打开口径。任何 production secret、production endpoint、broker connection、submit / cancel / replace、OMS、Event Store 或 Dashboard command surface 都必须在 CommandGateway、RiskEngine、ExecutionEngine、OMS、Event Store、kill switch / no-trade 和 validation gates 之后才可由后续 issue 明确授权；默认值始终是 productionTradingEnabledByDefault == false。

`GH-564-NO-OLD-BOUNDARY-AS-CURRENT`

No old boundary as current 指 public-read-only、paper-only、ExecutionClient future-gate 和 EMA-only 仍可作为已完成历史阶段、审计 evidence 或 compatibility evidence 出现，但不得再被写成 release v0.2.0 当前边界。当前 v0.2.0 口径必须同时包含 `productionCapabilityGatedNotMissing == true` 和 `oldPublicReadOnlyPaperOnlyEMAOnlyIsHistorical == true`。
...
Typed MessageBus envelope context 指 release v0.2.0 的 `EventEnvelope` metadata 必须能保存 `venue`、`productType` 和 `instrumentID`。该 metadata 用于区分 Binance Spot 与 Binance USDⓈ-M Perpetual 的同名交易对，不保存 endpoint、secret、account payload、broker order id、listenKey 或 live command 授权。

`GH-572-STRATEGY-RISK-EXECUTION-PORTFOLIO-PRODUCT-CONTEXT`

Strategy / Risk / Execution / Portfolio product context 指 strategy signal、risk evaluation、paper execution evidence 和 portfolio projection query 都能通过 MessageBus envelope 携带同一个 Binance instrument context。旧 payload 内的 `symbol` / `timeframe` 不能替代 release v0.2.0 envelope-level `productType`。

`GH-572-CORRELATION-CAUSATION-REPLAY-PARITY`

Correlation / causation replay parity 指 typed envelope 在 publish 和 replay 后必须保持相同 sequence、stream、typed context、correlationID 和 causationID。该 parity 只证明本地 append-only facts 可重放，不表示 production recovery、broker replay、OMS replay 或 incident replay runtime。

`GH-572-NO-LIVE-COMMAND-BUS`

No live command bus 指 typed MessageBus envelope 仍是本地事实 metadata，不是 external message broker、live command plane、ExecutionClient route、OMS request、broker command、signed endpoint request、account endpoint request、listenKey operation 或 Dashboard command surface。
...
Perpetual risk checks 指 release v0.2.0 中 Binance USDⓈ-M Perpetual 专属的本地 deterministic risk evidence。它消费 common RiskEngine allow evidence 和 public mark / funding read model，继续检查 leverage、liquidation、mark freshness、funding 和 reduce-only close；它不读取真实 account endpoint、不调用 signed endpoint、不连接 broker，也不授权真实订单。

`GH-580-LEVERAGE-LIQUIDATION-GATE`

Leverage / liquidation gate 指 Perp order intent 必须在本地通过 leverage cap 和 minimum liquidation distance ratio。超限只能输出 blocked evidence，不能 fallback 到 leverage adjust、margin action、OMS mutation、broker retry 或 Dashboard command。

`GH-580-MARK-FUNDING-RISK-GATE`

Mark / funding risk gate 指 stale mark price 必须 blocked，funding risk 必须支持 warning 和 block 两类 deterministic evidence。该 gate 只消费 public market read model，不读取账户 margin、不连接 production feed、不执行 funding debit / credit。

`GH-580-REDUCE-ONLY-CLOSE-VALIDATION`

Reduce-only close validation 指 reduce-only close 只有在 targetFlat、存在本地 position quantity 且 close quantity 不超过 position size 时才可通过。它不是 broker reduceOnly flag runtime，不提交真实 close order。
...
Product-aware OMS state machine 指 release v0.2.0 中消费 #581 Spot order intent 和 #582 Perpetual order intent 的本地 deterministic OMS evidence。它以 `InstrumentIdentity.productType` 区分 Spot 与 USDⓈ-M Perpetual lifecycle，生成 append-only event log 和 replay result；它不是 production OMS runtime、ExecutionClient request、broker order、live command 或 production trading 授权。

`GH-583-SPOT-LIFECYCLE`

Spot lifecycle 指 Binance Spot order intent 可在本地 OMS evidence 中从 `new` 进入 `accepted`、`submitted`、`partiallyFilled`、`filled` 或 `rejected` 等状态。该 lifecycle 只证明本地状态机和 event log 语义，不提交真实 Spot order。

`GH-583-PERP-LIFECYCLE`

Perp lifecycle 指 Binance USDⓈ-M Perpetual order intent 可在本地 OMS evidence 中从 `new` 进入 `accepted`、`submitted`、`partiallyFilled`、`filled` 或 `cancelled` 等状态。该 lifecycle 不执行 leverage / margin action，不提交真实 Perp order，也不打开 broker route。

`GH-583-ILLEGAL-TRANSITION-REJECTED`

Illegal transition rejected 指状态机必须机械拒绝不在 allowed graph 内的状态转移，例如 `submitted -> accepted`。拒绝非法 transition 不能被自动修正为其他 state，也不能绕过 Event Store / kill switch / no-trade 后续 gate。

`GH-583-REPLAY-RESTORES-ORDER-STATE`

Replay restores order state 指 replay 从 append-only OMS event log 恢复 terminal order state，并保持 source event log、orderID、source order intent 和 sequence evidence 可审计。Replay 不写 production order store，不调用 ExecutionClient，不连接 broker。
...
Binance USD-M Perpetual ExecutionClient adapter 指 release v0.2.0 中消费 GH-583 Perp OMS handoff identity 的本地 deterministic adapter evidence。它只生成 dry-run preview、Binance USD-M Perpetual testnet submit / cancel / replace request mapping 和 deterministic ack；它不是 broker gateway、production adapter、leverage / margin action、live command surface 或 production trading 授权。

`GH-585-SUBMIT-CANCEL-REPLACE-MAPPING`

Submit / cancel / replace mapping 指 Binance USD-M Perpetual adapter 对三类 command kind 的固定 request projection：host 固定为 `testnet.binancefuture.com`，endpoint 固定为 `/fapi/v1/order`，submit 使用 `POST`，cancel 使用 `DELETE`，replace 使用 `PUT`。该 mapping 只保存 redacted query item names / values，不暴露 signature value。

`GH-585-POSITIONSIDE-REDUCEONLY-MAPPING`

PositionSide / reduceOnly mapping 指 Perp request evidence 必须从 GH-583 Perp OMS handoff 继承 open long、open short、reduce-only close long、reduce-only close short 的方向约束。`positionSide` 与 `reduceOnly` 只作为 request mapping 参数存在，不授权 broker position sync、leverage action、margin action 或真实订单。

`GH-585-DRYRUN-EVIDENCE`

Dry-run evidence 指 adapter 在不执行 network call 的前提下生成完整 Perp request preview，并保留 issue identity、OMS handoff identity、clientOrderID、positionSide、reduceOnly、query item names 和 auditable mapping evidence。dry-run preview 不能被解释为 broker ack、execution report、fill、reconciliation 或 Portfolio update。

`GH-585-TESTNET-EVIDENCE-GATE`

Testnet evidence gate 指 adapter 只接受 testnet credential reference 和 Binance USD-M Perpetual testnet host，且 deterministic transport 只返回本地 ack evidence。该 gate 不读取 production secret，不连接 production endpoint，不连接 broker，也不把 testnet credential 升级成 production credential。

`GH-585-PRODUCTION-REJECTED-BY-DEFAULT`

Production rejected by default 指 GH-585 的默认值必须同时关闭 production endpoint、production secret read、broker gateway、live command surface、leverage / margin action 和真实 submit / cancel / replace。任何 production capability 只能留给后续 issue 在完整 gate 链下单独授权。
...
Perpetual Portfolio projection 指 release v0.2.0 中消费 GH-586 Perp normalized BrokerFill / position update 和 GH-575 mark / funding read model 的 deterministic projection evidence。它只计算本地 position、margin、PnL、funding 和 strategy attribution，不读取 production account endpoint，不同步 broker position，也不代表 production Portfolio runtime。

`GH-588-PERP-POSITIONAMT-ENTRY-MARK`

Perp positionAmt / entry / mark projection 指用 normalized Perp fill 的 latest position update、固定 entry price 和 public mark price 计算 USDⓈ-M Perpetual 仓位 notional。该 projection 不读取 broker position，不执行 leverage / margin action，也不把 Spot fill 混入 Perp Portfolio projection。

`GH-588-PERP-MARGIN-PROJECTION`

Perp margin projection 指基于 deterministic leverage 和 position notional 计算本地 margin requirement evidence。该 margin 不是 broker margin、account endpoint payload、buying power、settlement record 或 production margin ledger。

`GH-588-PERP-PNL-PROJECTION`

Perp PnL projection 指基于 deterministic entry price、fill price、mark price、commission 和 funding estimate 计算本地 realized / unrealized / net PnL evidence。该 PnL 不是 real PnL，不读取 broker statement、account endpoint、margin、leverage 或 reconciliation runtime。

`GH-588-PERP-FUNDING-PROJECTION`

Perp funding projection 指基于 GH-575 fresh funding rate 和 position notional 计算 funding payment estimate。该 estimate 不执行 funding settlement，不读取 broker statement，也不代表真实 funding fee。

`GH-588-PERP-STRATEGY-ATTRIBUTION`

Perp strategy attribution 指把 Perp BrokerFill projection attribution 绑定到 release v0.2.0 当前 active 的 EMA / RSI 策略。它不启用 non-EMA / non-RSI strategy，不创建 strategy runtime command，也不绕过 RiskEngine / ExecutionEngine / OMS gate。

`GH-588-NO-PRODUCTION-ACCOUNT-READ`

No production account read 指 GH-588 默认关闭 production account endpoint、production secret read、broker gateway、broker position sync、leverage / margin action、funding settlement、Portfolio runtime mutation、reconciliation runtime、Dashboard live command surface 和 production trading。

`TVM-RELEASE-V020-PERPETUAL-PORTFOLIO-PROJECTION`

Release v0.2.0 Perpetual Portfolio projection matrix 必须证明 GH-586 Perp BrokerFill + GH-575 mark / funding -> positionAmt / entryPrice / markPrice / margin / PnL / funding / EMA-RSI attribution / Portfolio financial state projection evidence 成立，且 `Portfolio` target 不直接依赖 `ExecutionClient`，projection 不绕过 CommandGateway / RiskEngine / ExecutionEngine / OMS / Event Store / kill switch / no-trade gate。

## GH-589 Aggregate Portfolio Attribution Terms

`GH-589-AGGREGATE-PORTFOLIO-ATTRIBUTION`

Aggregate Portfolio attribution 指 release v0.2.0 中消费 GH-587 Spot Portfolio projection 和 GH-588 Perpetual Portfolio projection 的 deterministic aggregate evidence。它只汇总 Spot + Perp exposure、PnL、funding 和 strategy attribution，不读取 production account endpoint，不同步 broker position，也不代表 production Portfolio runtime。

`GH-589-SPOT-PERP-EXPOSURE-SUMMARY`

Spot + Perp exposure summary 指 aggregate exposure 必须等于 Spot gross exposure + Perp gross exposure，并保留 Spot / USDⓈ-M Perpetual product identity。该 summary 不是 broker exposure、account endpoint payload、settlement record 或 production risk ledger。

`GH-589-EMA-RSI-ATTRIBUTION-SEPARATED`

EMA / RSI attribution separated 指 aggregate evidence 必须分别汇总 EMA 和 RSI 的 Spot / Perp attribution。该 separated attribution 不启用第三 active strategy，不创建 strategy runtime command，也不绕过 RiskEngine / ExecutionEngine / OMS gate。

`GH-589-FUNDING-LIQUIDATION-SUMMARY`

Funding / liquidation summary 指 aggregate evidence 必须展示 Perp funding rate、funding payment estimate 和 deterministic local liquidation reference。该 liquidation reference 不是 broker liquidation price、不是 margin call、不读取 broker margin，也不授权 leverage / margin action。

`GH-589-NO-PRODUCTION-PORTFOLIO-RUNTIME`

No production Portfolio runtime 指 GH-589 默认关闭 production account endpoint、production secret read、broker gateway、broker position sync、reconciliation runtime、Portfolio runtime mutation、Dashboard live command surface 和 production trading。
...
Verify-fast foundation + sample traces 指 release v0.2.0 的快速本地验收入口。它必须覆盖 foundation guard、typed message envelope、Spot / Perp sample market data trace、EMA / RSI Spot / Perp target exposure sample trace，并继续要求 CommandGateway、RiskEngine、ExecutionEngine、OMS、Event Store、kill switch 和 no-trade gate 不可绕过。

`GH-595-VERIFY-RELEASE-FULL-GATES-ALL-TRACES`

Verify-release full gates + all traces 指 release v0.2.0 的完整本地验收入口。它必须覆盖 foundation、sample traces、full gates 和 GH-592 的全部 15 条 golden trace，不代表 production endpoint 验证、broker statement、真实账户 checkpoint 或 production cutover。

`GH-595-MTPRO-VERIFY-FAST-GATE-PASS`

`mtpro verify-fast` gate pass 指 CLI fast verification 同时通过 GH-593 CLI surface 和 GH-595 fast coverage evidence。该输出不读取 secret、不连接 endpoint、不执行真实订单、不授权 production trading。

`GH-595-MTPRO-VERIFY-RELEASE-GATE-PASS`

`mtpro verify-release` gate pass 指 CLI release verification 同时通过 GH-592 golden trace catalog、GH-593 CLI surface 和 GH-595 release coverage evidence。该输出只证明本地 deterministic release evidence chain 完整，不替代 operator approval、risk approval、kill switch pass、broker connection 或 production cutover。

`GH-595-NO-PRODUCTION-VERIFY-SIDE-EFFECT`

No production verify side effect 指 verify-fast / verify-release 不得读取 production secret、连接 signed/account/private/broker endpoint、触碰 broker gateway、读取 account endpoint、提交 / 取消 / 替换真实订单、执行 production reconciliation 或启用 production trading。

`TVM-RELEASE-V020-VERIFY-FAST-RELEASE-GATES`

Release v0.2.0 verify-fast / verify-release gates matrix 必须证明 `verify-fast` 覆盖 foundation + sample traces，`verify-release` 覆盖 full gates + all traces，CLI 输出 `verificationIssue=GH-595` 和 coverage summary，并且 verification evidence 不绕过 CommandGateway / RiskEngine / ExecutionEngine / OMS / Event Store / kill switch / no-trade gate。

## GH-521 Release v0.1.0 Binance EMA Runtime Terms

`GH-521-RELEASE-V010-BINANCE-EMA-RUNTIME-CONTRACT`

Release v0.1.0 Binance EMA runtime contract 指 GitHub fallback queue GH-521 至 GH-541 的共同顶层合同。它只授权在对应 issue scope 内逐步建设 Binance + EMA 的 dry-run / testnet-first release evidence，不授权默认 production trading。

`GH-521-BINANCE-EMA-ACTIVE-SCOPE`

Binance EMA active scope 指 release v0.1.0 的唯一 active venue 是 Binance，唯一 active concrete strategy 是 EMA。非 Binance venue 和非 EMA concrete strategy 只能作为 future / forbidden / non-active scope 记录，不能写成 release v0.1.0 当前 active capability。

`GH-521-TESTNET-DRY-RUN-FIRST-GATE`

Testnet / dry-run first gate 指 release v0.1.0 的验证顺序必须先经过 local fixture / deterministic evidence、dry-run evidence 和 Binance testnet evidence，不能用 production secret、production endpoint 或 production broker connection 替代。

`GH-521-ACCEPTANCE-MATRIX`

Release v0.1.0 acceptance matrix 必须覆盖 DataClient / DataEngine / Cache、signed account read、private stream / account snapshot、Trader lifecycle、EMA proposal、RiskEngine pre-trade gate、ExecutionEngine / OMS、Binance ExecutionClient testnet submit / cancel / replace、execution report / broker fill parser、reconciliation / portfolio update、Dashboard live monitoring、controlled command surface、kill switch / no-trade / rollback、dry-run / testnet validation、no-default-production-trading automation guard、release docs、operator runbook、validation matrix、stage audit input、final Stage Code Audit 和 Root Docs Refresh。

`GH-521-NO-DEFAULT-PRODUCTION-TRADING`

No-default-production-trading 指 release v0.1.0 默认状态必须保持 production trading、production endpoint、production secret read、production submit / cancel / replace、production OMS、production Dashboard command surface、non-Binance venue 和 non-EMA active strategy 全部关闭。
...
Release v0.1.0 real target smoke coverage 指 GH-523 通过真实 public API import / use release modules，证明 `DomainModel`、`MessageBus`、`Database`、`DataClient`、`DataEngine`、`Cache`、`Trader`、`TraderStrategies`、`Portfolio`、`RiskEngine`、`ExecutionEngine`、`ExecutionClient` 和 `Dashboard` 不是只存在于 `Package.swift` 文本里。该 coverage 只证明 module surface 可用，不等同于 runtime implementation。
...
Binance private stream / account snapshot runtime 指 release v0.1.0 中 DataClient 拥有的 read/update evidence path：先用 testnet / local fixture-first listenKey lifecycle request 建立 redacted listenKey reference，再消费 mock/source-driven private stream frames，把 `outboundAccountPosition` 和 `balanceUpdate` 映射为 canonical account / balance / position read-model records。Raw listenKey、raw private payload、secret、broker state、execution report 和 order update 均不得进入 read model。
...
EMA strategy proposal runtime 指 release v0.1.0 中唯一 active concrete strategy `EMA` 的 signal-to-proposal path。它可以从 live-read compatible market bars / `EMACrossSignalSample` 生成 paper-only `PaperActionProposal`，但不能生成 executable order command。

`GH-528-EMA-SIGNAL-TO-PAPER-PROPOSAL`

EMA signal-to-paper-proposal 表示 `EMACrossStrategyContract` 产生的 `StrategySignalEvent` 只能映射为 `PaperActionProposal`。`long` 信号可以生成本地 `buy` paper intent，`flat` 信号只能生成 hold / zero-quantity paper intent；两者都不表示 broker order side、margin side、short side 或 live command。

`GH-528-RISKENGINE-CONSUMABLE-PROPOSAL`

RiskEngine consumable proposal 表示 GH-528 evidence 必须同步生成与 proposal 一致的 `RiskEvaluationQuery`，让 RiskEngine 后续按 paper mode 消费 proposal / query。该语义不授权 Trader 或 Strategy 跳过 RiskEngine，不授权 ExecutionEngine command，不授权 broker submit / cancel / replace。
...
ExecutionEngine OMS state machine 指 release v0.1.0 中 ExecutionEngine 对 #529 RiskEngine decision evidence 的本地订单生命周期证据。它覆盖 `new`、`accepted`、`rejected`、`canceled`、`replaced`、`filled` 状态，但只生成 deterministic event log / audit evidence，不调用 ExecutionClient，不连接 broker，不写 production order store，不授权 production OMS runtime。

`GH-530-RISK-APPROVED-ORDER-INTENT`

Risk-approved order intent 表示只有 #529 `approved` risk decision 可以进入 ExecutionEngine local lifecycle。该 intent 不是 ExecutionClient request，不是 broker order，也不是 submit / cancel / replace command；#529 rejected / blocked decision 只能进入 rejected path。

`GH-530-OMS-EVENT-LOG-AUDIT-EVIDENCE`

OMS event log audit evidence 表示 GH-530 的状态变更必须是 append-only、deterministic、audit-only。它可以证明 accepted / canceled / replaced / filled / rejected path，但不能解释为 production order store、execution report parser、broker fill parser、reconciliation runtime 或 Dashboard command surface。

`GH-530-NO-PRODUCTION-OMS-RUNTIME`

No production OMS runtime 表示 release v0.1.0 当前仍保持 production OMS disabled by default。ExecutionEngine lifecycle evidence 不能打开 production secret、production endpoint、broker gateway、real submit / cancel / replace、production reconciliation 或 production cutover。
...
Dashboard live monitoring surface 指 release v0.1.0 中 Dashboard 报告区和 shell smoke 对已完成 release evidence 的 read-model-only summary。它显示 #526 account/private stream、#528 EMA proposal、#529 RiskEngine gate、#530 ExecutionEngine / OMS、#532 execution report / broker fill 和 #533 Portfolio reconciliation 状态，但不直接消费这些 targets 的 runtime object。

`GH-534-CONNECTION-HEALTH-READ-MODEL`

Connection health read model 表示 Dashboard 只能显示 fresh / stale / blocked / unavailable 等连接健康摘要，不打开 network reconnect、不创建 connection manager、不触发 runtime connection。

`GH-534-ACCOUNT-PRIVATE-STREAM-STATUS`

Account/private stream status 表示 Dashboard 只能显示 redacted account / balance / position read-model identity，不暴露 raw private payload、listenKey value、secret、account endpoint response 或 broker state。

`GH-534-TRADER-EMA-RISK-EXECUTION-PORTFOLIO-SUMMARY`

Trader/EMA/Risk/Execution/Portfolio summary 表示 Dashboard 可以展示 proposal、risk gate、OMS lifecycle、broker fill parser 和 Portfolio reconciliation 的 evidence 状态，但不能把这些状态转成 command surface、trading button、live command 或 order form。

`GH-534-READ-MODEL-ONLY-NO-COMMAND-SURFACE`

Read-model-only no-command surface 表示 GH-534 必须保持 Dashboard target 不新增 runtime target dependency，不提供 secret editor、broker connect、account connect、submit / cancel / replace command 或 production trading authorization。

`TVM-RELEASE-V010-DASHBOARD-LIVE-MONITORING-SURFACE`

Release v0.1.0 Dashboard live monitoring matrix 必须证明 Dashboard 可以读取 release live evidence summary，同时 command surface、trading button、live command、order form、secret editor、runtime object consumption、network connection、account payload exposure、broker connection 和 production trading 均保持关闭。

## GH-535 Dashboard Controlled Command Surface Terms

`GH-535-DASHBOARD-CONTROLLED-COMMAND-SURFACE`

Dashboard controlled command surface 指 release v0.1.0 中 Dashboard 报告区和 shell smoke 对 submit / cancel / replace command entry 的受控展示模型。它可以显示 command entry、dry-run gate、Binance testnet gate 和 production disabled explanation，但所有 entry 默认 no-trade，且不调用 ExecutionClient、broker、OMS 或真实订单 runtime。

`GH-535-DEFAULT-NO-TRADE-COMMAND-ENTRY`

Default no-trade command entry 表示 Dashboard 中的 command entry 默认不可执行，`commandSurfaceEnabled == false`，任何生产下单都必须继续被禁止。

`GH-535-DRYRUN-TESTNET-GATE`

Dry-run / testnet gate 表示 Dashboard 只能展示受控 dry-run / Binance testnet 路径标签；它不读取 production secret、不连接 production endpoint，也不把 testnet gate 扩大成 production broker connection。

`GH-535-PRODUCTION-DISABLED-BY-DEFAULT`

Production disabled by default 表示用户必须能看到 production 被禁止的原因；缺少 explicit release gate、operator confirmation、risk approval 和 kill switch pass 时，production submit / cancel / replace 永远不可用。

`GH-535-NO-RISK-EXECUTION-KILLSWITCH-BYPASS`

No RiskEngine / ExecutionEngine / kill switch bypass 表示 Dashboard controlled command surface 必须显示 RiskEngine、ExecutionEngine / OMS、operator confirmation 和 kill switch gate requirement，并且不能绕过这些 gate 授权交易执行。
