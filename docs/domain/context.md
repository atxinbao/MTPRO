...
MTP-221 的 dependency direction 是 `Workbench -> Core / Persistence read-model and ViewModel exports only`、`Dashboard -> Workbench`、`App -> Workbench compatibility re-export`。MTP-222 后 `App` 只作为 retained compatibility export 解释既有 import surface，不再表达 Workbench source owner 或 Dashboard dependency owner。
...
Current target graph 指 `DomainModel`、`MessageBus`、`Database`、`DataClient`、`Cache`、`DataEngine`、`Portfolio`、`RiskEngine`、`ExecutionClient`、`ExecutionEngine`、`TraderStrategies`、`Trader`、`Workbench` 和 `Dashboard`。Current UI dependency direction 是 `Dashboard -> Workbench`；`App` 只是 `Workbench` compatibility re-export。
...
Stale active anchor retirement 表示 active source anchors 必须指向 `Sources/Trader/Strategies/EMA/`、`Sources/Trader/Coordination/RiskBinding/`、`TraderStrategies`、`Trader`、`Workbench` 和 `Dashboard -> Workbench`。旧 peer-level `Sources/Strategies/` 和旧 first-level `Sources/Trader/StrategyBindings/` 不得回流为 active source root、Package root、test root 或 target name。
