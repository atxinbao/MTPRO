...
它不是执行授权，不替代 Linear Project / Issue，不替代 `GOAL.md`、`docs/architecture.md`、`docs/roadmap.md`，不修改 Linear status，不启动额外调度服务，不创建 PR。
...
- Source `docs/architecture.md`：`/Users/mac/Documents/MTPRO/docs/architecture.md`
- Source `docs/roadmap.md`：`/Users/mac/Documents/MTPRO/docs/roadmap.md`
...
`@000 / AIE` 不替代 Human decision，不绕过 Linear configured executable issue，不替代 `@001 / PLN` 的下一阶段 Project Planning，不替代 `@002 / PAR` queue 调度或 Linear 外 reference 研究角色。Complete Blueprint Design 不创建 Linear Project / Issue，不修改 Linear status，不推进 `Todo`，不启动额外调度服务，不写业务代码。

`@003 / PRD`、`@004 / DSG`、`@005 / ARC` 是 Linear 外的 reference / root docs 角色。它们用于外部参考项目研究、产品 / 设计 / 架构差距分析，以及 `GOAL.md`、`docs/architecture.md`、`docs/environment.md`、`docs/roadmap.md` 的 delta proposal。它们不创建 Linear Project / Issue，不修改 Linear status，不推进 `Todo`，不启动额外调度服务，不写业务代码。
...
| `003` | `PRD` | Product Reference Lead | Linear 外产品参考研究、用户路径、工作台能力、`GOAL.md` / `docs/roadmap.md` / `docs/product/*` delta proposal |
...
| `007` | `OPS` | Operations | 本地环境、运行、GitHub / Linear 自动化可用性 |

Codex Execution Agent 和 GitHub PR Automation 是流程工具 / 执行层 actor，按名称调用，不占用 `@003`、`@004`、`@005` 编号。
...
| 角色编号 | 当前职责 | 产物 | 禁止 |
| --- | --- | --- | --- |
| `@003 / PRD` | 阅读外部参考项目的产品能力、用户路径、工作台流程和 acceptance 组织方式，提炼 MTPRO 应该采用 / 不采用的产品决策 | `docs/reference/*/product-reference.md`、`GOAL.md` / `docs/roadmap.md` / `docs/product/*` delta proposal | 不创建 Linear Project / Issue，不推进 `Todo`，不写业务代码，不把参考项目能力直接当成 MTPRO scope |
| `@004 / DSG` | 阅读外部参考项目的信息架构、页面组织、操作路径、状态表达和工作台结构，提炼 Dashboard / Workbench / ViewModel 设计建议 | `docs/reference/*/design-reference.md`、Product Surface / Frontend ViewModel delta proposal | 不写 UI 代码，不启动额外调度服务，不把 wireframe / 页面建议当作执行授权 |
| `@005 / ARC` | 阅读外部参考项目源码、官方文档和 API 文档，提炼 event-driven、replay、adapter、runtime、portfolio、risk、execution 等系统结构参考 | `docs/reference/*/architecture-reference.md`、`api-reference-map.md`、`docs/architecture.md` / `docs/environment.md` / `docs/contracts/*` delta proposal | 不重写架构，不引入 Live trading / signed endpoint / broker action，不把外部 API 直接映射为实现任务 |
...
| System Architect | 维护 MTPRO 架构边界、模块关系、事件流和自动化边界 | `docs/architecture.md` / module boundary / API boundary | Human + ChatGPT + Parent Codex | partial |
...
| Automation / Runtime Operations Engineer | 维护 GitHub PR Automation、Linear gate、本地运行、credential 边界、runtime readiness 和环境健康 | automation readiness / workflow / Post-Issue Ledger / macOS build-run loop | Parent Codex | partial |
...
| Operations | Automation / Runtime Operations Engineer / Parent Codex Supervisor | GitHub PR Automation、Linear gate、Post-Issue Ledger、credential、runtime readiness | UI / runtime 阶段前需要补本地运行和 telemetry evidence |
...
| `docs/architecture.md` | System Architect | Backend Engineer / Data Designer / DevOps | Bootstrap PR |
...
| Validation plan | QA / Trading Validation Engineer | Parent Codex Supervisor | Codex Execution Agent |
...
- Project Planning Facilitator 只负责 Project / Issue 草案、顺序、依赖、validation、evidence 和 Linear 写入准备；不得操作 `Backlog` -> `Todo`，不得启动额外调度服务。
...
- Parent Codex 负责把唯一 eligible issue 推进到 `Todo`，并确认 WIP=1、依赖、contract 和 active conflict。
- Codex Execution Agent 负责当前 issue scope 内实现、验证、PR、GitHub auto-merge handoff 和本地 handoff evidence。
- GitHub PR Automation 负责 required checks、auto-merge、squash merge、branch cleanup 和 Linear bot auto Done。
- Post-Issue Ledger 只提供 PR merge / Linear Done 后的本地只读摘要，不依赖额外调度或图谱服务。
...
| Automation / Runtime Operations Engineer partial | GitHub PR Automation、Linear gate、runtime readiness 可能靠人工记忆 | 自动化和 runtime issue 必须保留 readiness evidence、telemetry 和 fallback notes |
...
- [ ] 缺口不会被解释为执行授权。
- [ ] Project Role Map 不替代 `GOAL.md`、`docs/architecture.md`、`docs/roadmap.md`、Linear Project 或 Linear issue。
- [ ] Project Role Map 不修改 Linear status。
- [ ] Project Role Map 不启动额外调度服务、图谱更新服务或 GitHub PR Automation。
