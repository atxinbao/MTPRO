...
它不是执行授权，不替代 Linear Project / Issue，不替代 `GOAL.md`、`architecture.md`、`docs/roadmap.md`，不修改 Linear status，不启动额外调度服务，不创建 PR。
...
`@003 / PRD`、`@004 / DSG`、`@005 / ARC` 是 Linear 外的 reference / root docs 角色。它们用于外部参考项目研究、产品 / 设计 / 架构差距分析，以及 `GOAL.md`、`architecture.md`、`environment.md`、`docs/roadmap.md` 的 delta proposal。它们不创建 Linear Project / Issue，不修改 Linear status，不推进 `Todo`，不启动额外调度服务，不写业务代码。
...
- [ ] 缺口不会被解释为执行授权。
- [ ] Project Role Map 不替代 `GOAL.md`、`architecture.md`、`docs/roadmap.md`、Linear Project 或 Linear issue。
