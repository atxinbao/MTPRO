...
| `docs/environment.md` | 无需新增 secret / credential / signed endpoint / listenKey / broker account / production operations 配置；统一验证入口仍是 `bash checks/run.sh`。 |
| `docs/architecture.md` | 同步 current source tree 已把 active strategy layout 收口为 EMA-only，并明确 SwiftPM target graph 仍保留 compatibility envelope；不得把目录/validation 收口误写成 runtime implementation 完成。 |
