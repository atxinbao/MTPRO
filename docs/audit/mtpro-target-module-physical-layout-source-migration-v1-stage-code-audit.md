...
| `architecture.md` | 同步 target module physical layout 已完成：DomainModel / MessageBus / DataClient / DataEngine / Cache / Database / Strategies / Trader / Portfolio / RiskEngine / ExecutionEngine / ExecutionClient / Workbench / Dashboard 已进入 target source directories；不得把 signed/account/broker/OMS/live command 模块写成当前 runtime。 |
