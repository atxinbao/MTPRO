...
| `docs/architecture.md` | Core / App / Dashboard 边界继续成立；scenario replay evidence 沿 local deterministic fixture -> replay evidence -> quality / report input version -> read model / ViewModel -> Workbench evidence surface 流动，不读取 adapter、Runtime object、SQLite / DuckDB schema、真实账户 / broker state 或 production operations state。 |
