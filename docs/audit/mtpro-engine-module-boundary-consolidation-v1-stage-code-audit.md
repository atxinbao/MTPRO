...
| `architecture.md` | 同步 target engine/module boundary 已完成：DataClient / DataEngine / MessageBus / Cache / Database / Strategies / Trader / Portfolio / RiskEngine / ExecutionEngine / ExecutionClient / Workbench / Dashboard / Future Live PRO Console 目标边界已进入 L4 planning input；不得把 signed/account/broker/OMS/live command 模块写成当前 runtime。 |
