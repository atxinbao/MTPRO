...
本文不修改 Linear，不创建 Project / Issue，不推进 Todo，不启动 Symphony，不写业务代码，不直接修改 `ARCHITECTURE.md` 或 `ROADMAP.md`，不实现 UI。
...
## 对 `ARCHITECTURE.md` 的候选 delta proposal

不直接修改 `ARCHITECTURE.md`。建议未来由 `@000 / AIE` 或 `@001 / PLN` 汇总后评估是否加入：
...
不直接修改 `ROADMAP.md`。建议未来汇总时只作为候选规划输入：
