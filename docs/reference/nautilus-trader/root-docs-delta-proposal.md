...
本文档不直接修改 `GOAL.md`、`environment.md`、`architecture.md`、`docs/roadmap.md` 或 `docs/product/*` / `docs/contracts/*`。它先作为 Human + `@000 / AIE` 的 Complete Blueprint Design 输入，再作为 Human + `@001 / PLN` 后续规划输入。不写 Linear、不创建 Project / Issue、不推进 `Todo`、不启动 Symphony、不写业务代码。
