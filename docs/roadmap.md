...
| Trader EMA Strategy Layout Consolidation before L4 | Done / not counted in old denominator | 收口 current active concrete strategy only `EMA`，canonical active path only `Sources/Trader/Strategies/EMA/`；非 EMA strategy 只能作为 future candidate / historical evidence / compatibility debt，不得写成当前 active strategy。 | EMA-only active source layout、OrderBookImbalance Core research evidence、Trader Coordination RiskBinding boundary、deterministic EMA-only path validation、validation matrix 和 compatibility envelope closeout。 | `docs/audit/mtpro-trader-ema-strategy-layout-consolidation-v1-stage-code-audit.md` 已记录完整 Project closure、PR #327 至 #334 evidence、validation、Root Docs Delta 和 forbidden capability audit；不新增 SwiftPM target，不授权 Strategy runtime、Trader runtime、ExecutionClient、broker、OMS、Live PRO Console 或 live command。 |
