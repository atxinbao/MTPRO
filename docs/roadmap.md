...
当前 active construction scope 是 `MTPRO Release v0.13.0 Local Evidence-driven Readiness Engine` 的 #994 contract gate。它使用 GitHub fallback issue queue GH-994 至 GH-1005 作为唯一队列来源；Linear 不参与本阶段执行。#994 只定义 inputs、outputs、evidence roots、schema contracts、lifecycle order、fail-closed behavior 和 artifact -> policy -> manifest -> bundle -> registry -> diff chain；#995 至 #1005 在 #994 PR merge、required checks success、issue closed / done、main fast-forward 和 fresh WIP=1 preflight 前保持 blocked。#994 不实现 evidence intake runtime，不创建 tag / GitHub Release，不授权 production cutover。
...
Next recommended maturity slice：`release/v0.13.0` / #994 `V013-01 Define local evidence-driven readiness engine contract` active contract gate。

Next maturity planning candidate：#995 至 #1005 仍 blocked by #994；real broker / production trading / next Project 仍必须经 Human + `@001 / PLN` 重新规划。
