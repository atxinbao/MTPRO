...
| 目标切片 | 进入前置 | 当前状态 |
| --- | --- | --- |
| 实盘交易基础边界 | Human 独立决策、独立 Project Definition、secret / signed endpoint / account endpoint / broker adapter / real order lifecycle gates | Pending / gated |
| 实盘监控台 | 已定义 live runtime health、connection、market stream、order stream、error、latency 和 operations evidence | Pending / gated |
| 实盘执行控制 | 已定义 real order submit / cancel / replace、execution report、reconciliation 和 incident fallback | Pending / gated |
| 实盘风险控制 | 已定义 live pre-trade risk、熔断、禁交易状态、stop controls 和 operations readiness | Pending / gated |
