...
## MTP-58 Market Data Replay Projection Consistency 观察面

日期：2026-05-20

执行者：Codex

Market data replay operations 在当前事项中新增 `MarketDataReplayProjectionSnapshotConsistencySummary`，作为后续 Report / Dashboard / Event Timeline 可消费的稳定 read model evidence。

当前可观察字段：

- batch id、replay run id、symbol、interval 和 time window。
- freshness status 和 freshness summary。
- append-only event log stream、event log sequences、replay result sequences 和 record count。
- replay output summary、cache snapshot summary 和 DuckDB analytical projection summary。
- market-only replay 下 SQLite runtime projection 的空快照计数。
- read-model-only、schema non-exposure、adapter / runtime non-exposure 和 no trading authorization flags。

边界：

- 当前只定义 Runtime 层 consistency read model，不新增 SwiftUI 页面或 Report 接入。
- 当前不把 SQLite / DuckDB schema、SQL、ORM model、Runtime object、adapter request 或 persistence adapter direct read 暴露给前端。
- 当前不包含 signed endpoint、account endpoint、listenKey、broker action、Live trading 或真实订单行为。
