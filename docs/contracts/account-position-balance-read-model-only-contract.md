...
- `Sources/Dashboard/ReadModels/App.swift` 必须把 APB surface 接入 `ReportReadModel` / `ReportViewModel` / `DashboardViewModel` 的 read-model-only source chain。
- `Sources/Dashboard/Events/PaperWorkflowEvidenceExplorer.swift` 必须包含 `accountPositionBalanceReadModelOnlySurface` section、coverage flag 和三条 read-model-only timeline items。
