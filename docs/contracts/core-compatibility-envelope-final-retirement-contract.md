...
| `Core` | `Sources/Core/LiveAdapterCapabilityIsolationBoundary.swift` | `ExecutionClient` | compatibility bridge that delegates capability-isolation evidence to the ExecutionClient-owned implementation | move remaining callers to `ExecutionClient` and retire the Core bridge |
