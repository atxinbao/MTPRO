...
`RSI`、`OrderBookImbalance`、`Momentum` 和 `MeanReversion` 只能作为 future strategy candidate / future-gated strategy label 出现。它们不得作为当前 active concrete strategy、不得进入 current active strategy source root、不得进入 Package.swift active strategy root、不得进入 Trader runtime、ExecutionClient request、OMS order、broker order、live command、order form 或 trading button。MTP-201 已将 `Sources/Trader/Strategies/OrderBookImbalance/` 退休；现有 OrderBookImbalance 类型只保留在 `Sources/Core/Research/OrderBookImbalanceResearchEvidence.swift` 作为 historical research / parity / persistence evidence，不是 current active concrete strategy source。
