...
GH-421 将 real target ownership 验证从分散的 per-area focused tests 收紧为一条全架构 deterministic smoke chain。`TargetGraphTests/testGH421AllArchitectureTargetsExposeIndependentRealAPISmokeCoverage` 必须直接 import / use:

- `DomainModel`：source ownership、symbol、timeframe、bar、identifier、quantity、price。
- `MessageBus`：foundation stream、append-only journal、proposal / sizing / exposure vocabulary。
- `Database`：foundation checkpoint。
- `DataClient` / `DataEngine` / `Cache`：public read-only data source、cache read model snapshot、read-only replay plan。
- `TraderStrategies` / `Trader`：EMA-only strategy contract 和 Trader account context future gate。
- `Portfolio` / `RiskEngine` / `ExecutionClient` / `ExecutionEngine`：paper-only exposure projection、pre-trade decision、ExecutionClient future gate、paper execution handoff。
- `Dashboard`：Dashboard read-model-only target boundary。

验收口径：测试必须证明所有 architecture targets 能暴露并组合真实 public APIs，不得只依赖 `Package.swift` string、target name 或 `TargetGraph` boundary anchor；同时必须保持 no live / no broker / no OMS / no real order / no L4 guard。

验证命令：

- `swift test --filter TargetGraphTests/testGH421AllArchitectureTargetsExposeIndependentRealAPISmokeCoverage`
- `swift test --filter TargetGraphTests`
- `git diff --check`
- `bash checks/automation-readiness.sh`
- `bash checks/run.sh`
