...
- `PaperWorkflowDashboardInformationArchitecture`：固定 Dashboard 观察面、session-level controls 和 forbidden capability。
...
MTP-48 只新增本地 Paper session-level control Command Model，不把 control 串入 event boundary，也不启动 runtime side effect。

契约结构：

- `PaperSessionLocalControlCommand`：表达已接受的本地 session control intent。
- `PaperSessionLocalControlAction`：只支持 `start` / `pause` / `close` / `reset`。
- `PaperSessionLocalControlValidation`：把 raw request 分为 accepted command 或 rejected reason。
- `PaperSessionLocalControlRejectedReason`：固定非 session-level、order-level、real order 和 broker-facing 拒绝边界。

契约要求：

- accepted command 只能作用于本地 Paper session，不能代表真实交易授权。
- validation 必须拒绝 `submit` / `cancel` / `replace`、broker action、signed endpoint、account endpoint、listenKey、Live trading 和 order-level command。
- command capability flags 必须全部为 false，Codable 解码不得绕过这些不变量。

本契约不包含：

- session-level control -> paper-only event boundary 串联。
- Paper session facts 写入。
- UI 控件或 Event Timeline 实现。
- order submit / cancel / replace。
- OMS。
- broker / exchange side effect。
