...
## MTP-47 Paper Workflow Dashboard IA ViewModel 契约
...
- `PaperWorkflowDashboardInformationArchitecture.deterministicFixture`：以可编码 fixture 证明当前 Dashboard IA 仍是 read-model-only 合同，不提前实现命令、控件或 Event Timeline。

边界确认：

- 该合同只定义后续 Dashboard / ViewModel / Command Model / Event Timeline 的输入边界。
- 当前 issue 不新增 SwiftUI 页面字段，不新增按钮，不新增 `Command` case，不写 event log。
- Dashboard IA 仍只引用 `DashboardSection` 和 `ViewModelSourceContract`，不暴露 SQLite / DuckDB schema、SQL、ORM model、runtime object 或 adapter request。
