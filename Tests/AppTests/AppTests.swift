...
final class AppTests: XCTestCase {
    ...
    func testGH1074DashboardTestnetExecutionStatusSurfaceShowsReadOnlyStatusWithoutCommands() throws {
        // 测试场景：GH-1074 Dashboard 只展示 v0.15.0 Spot Testnet execution evidence 状态。
        // 验证目的：submit / cancel / cancel-replace、OMS state、reconciliation 和 failure reason
        // 全部来自本地 read-model artifact；Dashboard 不能暴露交易按钮、命令面或 order form。
        let snapshot = DashboardShellSnapshot(viewModel: try makeDashboardViewModel())
        let surface = snapshot.releaseV0150DashboardTestnetExecutionStatusSurface

        XCTAssertEqual(surface.issueID, "GH-1074")
        XCTAssertEqual(surface.upstreamIssueIDs, ["GH-1072", "GH-1073"])
        XCTAssertEqual(surface.previousIssueID, "GH-1073")
        XCTAssertEqual(surface.releaseVersion, "v0.15.0")
        XCTAssertTrue(surface.source.isReadModelOnly)
        XCTAssertTrue(surface.boundaryHeld)
        XCTAssertTrue(surface.input.inputHeld)
        XCTAssertEqual(
            surface.input.sourceEvidenceType,
            "ReleaseV0150DashboardTestnetExecutionStatusReadModel"
        )
        XCTAssertEqual(surface.input.sourceActionKinds, ReleaseV0150DashboardTestnetExecutionStatusAction.allCases)
        XCTAssertEqual(surface.input.reconciliationStatus, "passed")
        XCTAssertEqual(surface.input.failureReasons, ["none"])
        XCTAssertEqual(surface.input.venueName, "Binance")
        XCTAssertEqual(surface.input.executionProductScope, "Binance Spot Testnet")
        XCTAssertEqual(surface.rows.map(\.action), ReleaseV0150DashboardTestnetExecutionStatusAction.allCases)
        ...
        XCTAssertEqual(metricValue("v0.15 execution status rows", in: surface.metrics), "3")
        XCTAssertEqual(metricValue("v0.15 execution actions", in: surface.metrics), "submit,cancel,cancel-replace")
        XCTAssertEqual(metricValue("v0.15 OMS state", in: surface.metrics), "gh-1072-v0150-oms-state-snapshot")
        XCTAssertEqual(metricValue("v0.15 reconciliation", in: surface.metrics), "passed")
        XCTAssertEqual(metricValue("Boundary", in: surface.metrics), "confirmed")
        XCTAssertTrue(surface.details.contains("Dashboard command surface: none"))
        XCTAssertTrue(surface.details.contains("Trading button: none"))
        XCTAssertTrue(surface.details.contains("Order form: none"))
        ...
        XCTAssertTrue(snapshot.isReadModelOnly)
        XCTAssertTrue(snapshot.viewModelSources.allSatisfy(\.isReadModelOnly))
        XCTAssertTrue(snapshot.smokeSummary.contains("releaseV0150ExecutionStatusRows=3"))
        XCTAssertTrue(snapshot.smokeSummary.contains("releaseV0150ExecutionActions=submit,cancel,cancel-replace"))
        XCTAssertTrue(snapshot.smokeSummary.contains("releaseV0150ExecutionOMSState=gh-1072-v0150-oms-state-snapshot"))
        XCTAssertTrue(snapshot.smokeSummary.contains("releaseV0150ExecutionReconciliation=passed"))
        XCTAssertTrue(snapshot.smokeSummary.contains("releaseV0150ExecutionBoundary=confirmed"))
        ...
        let encoder = JSONEncoder()
        encoder.outputFormatting = [.sortedKeys]
        let artifactJSON = try encoder.encode(ReleaseV0150DashboardTestnetExecutionStatusLocalArtifactInput())
        let artifactInput = try ReleaseV0150DashboardTestnetExecutionStatusSurfaceViewModel
            .localReadModelArtifactInput(fromJSON: artifactJSON)
        let loadedSurface = try DashboardShellSnapshot
            .releaseV0150DashboardTestnetExecutionStatusSurface(fromLocalReadModelJSON: artifactJSON)
        let loadedSnapshot = DashboardShellSnapshot(
            viewModel: try makeDashboardViewModel(),
            releaseV0150DashboardTestnetExecutionStatusSurface: loadedSurface
        )

        XCTAssertTrue(artifactInput.inputHeld)
        XCTAssertEqual(artifactInput.artifactID, "gh-1074-dashboard-testnet-execution-status")
        XCTAssertEqual(
            artifactInput.schema,
            ReleaseV0150DashboardTestnetExecutionStatusLocalArtifactInput.schemaID
        )
        XCTAssertEqual(artifactInput.releaseVersion, "v0.15.0")
        XCTAssertTrue(loadedSurface.boundaryHeld)
        XCTAssertTrue(loadedSnapshot.isReadModelOnly)
        XCTAssertTrue(loadedSnapshot.smokeSummary.contains("releaseV0150ExecutionBoundary=confirmed"))

        let invalidPath = try encoder.encode(
            ReleaseV0150DashboardTestnetExecutionStatusLocalArtifactInput(
                relativePath: "../escape/dashboard-testnet-execution-status.json"
            )
        )
        XCTAssertThrowsError(
            try ReleaseV0150DashboardTestnetExecutionStatusSurfaceViewModel
                .localReadModelArtifact(fromJSON: invalidPath)
        )

        let commandSurface = try encoder.encode(
            ReleaseV0150DashboardTestnetExecutionStatusLocalArtifactInput(
                surface: ReleaseV0150DashboardTestnetExecutionStatusSurfaceViewModel(
                    dashboardCommandSurfaceEnabled: true
                )
            )
        )
        XCTAssertThrowsError(
            try DashboardShellSnapshot.releaseV0150DashboardTestnetExecutionStatusSurface(
                fromLocalReadModelJSON: commandSurface
            )
        )
        ...
        for anchor in ReleaseV0150DashboardTestnetExecutionStatusSurfaceViewModel.requiredValidationAnchors {
            XCTAssertTrue(surface.validationAnchors.contains(anchor), "\(anchor) must be part of GH-1074 surface")
        }
    }

    func testGH1108DashboardArtifactBackedExecutionViewShowsLocalArtifactsWithoutCommands() throws {
        // 测试场景：GH-1108 Dashboard 必须展示 v0.16.0 本地 operator artifact-backed execution view。
        // 验证目的：action sequence、record checksum、artifact path 和 #1107 reconciliation result
        // 全部是只读 read model；Dashboard 不能出现 command handler、交易按钮或 order form。
        let snapshot = DashboardShellSnapshot(viewModel: try makeDashboardViewModel())
        let surface = snapshot.releaseV0160DashboardArtifactBackedExecutionView

        XCTAssertEqual(surface.issueID, "GH-1108")
        XCTAssertEqual(surface.upstreamIssueIDs, ["GH-1106", "GH-1107"])
        XCTAssertEqual(surface.previousIssueID, "GH-1107")
        XCTAssertEqual(surface.releaseVersion, "v0.16.0")
        XCTAssertTrue(surface.source.isReadModelOnly)
        XCTAssertTrue(surface.boundaryHeld)
        XCTAssertTrue(surface.input.inputHeld)
        XCTAssertEqual(
            surface.input.sourceEvidenceType,
            "ReleaseV0160DashboardArtifactBackedExecutionReadModel"
        )
        XCTAssertEqual(surface.input.venueName, "Binance")
        XCTAssertEqual(surface.input.executionProductScope, "Binance Spot Testnet")
        XCTAssertEqual(surface.input.actionSequence, ReleaseV0160DashboardArtifactBackedExecutionAction.allCases)
        XCTAssertEqual(surface.rows.map(\.action), ReleaseV0160DashboardArtifactBackedExecutionAction.allCases)
        XCTAssertEqual(surface.rows.map(\.sequence), Array(1...3))
        XCTAssertEqual(surface.rows.map(\.artifactRecordID), surface.input.artifactRecordIDs)
        XCTAssertEqual(surface.rows.map(\.artifactChecksum), surface.input.artifactRecordChecksums)
        XCTAssertEqual(surface.rows.map(\.artifactRelativePath), surface.input.artifactRelativePaths)
        XCTAssertTrue(surface.rows.allSatisfy(\.rowHeld))
        XCTAssertEqual(surface.visibleRowCount, 3)
        XCTAssertEqual(surface.actionLabels, ["submit", "cancel", "status"])
        XCTAssertTrue(surface.localArtifactBackedRowsVisible)
        XCTAssertTrue(surface.actionSequenceVisible)
        XCTAssertTrue(surface.checksumsVisible)
        XCTAssertTrue(surface.omsReconciliationResultVisible)
        XCTAssertTrue(surface.readOnly)
        XCTAssertFalse(surface.dashboardDependsOnExecutionClientTarget)
        XCTAssertFalse(surface.dashboardCommandSurfaceEnabled)
        XCTAssertFalse(surface.tradingButtonVisible)
        ...
        XCTAssertEqual(metricValue("v0.16 artifact rows", in: surface.metrics), "3")
        XCTAssertEqual(metricValue("v0.16 action sequence", in: surface.metrics), "submit,cancel,status")
        XCTAssertEqual(metricValue("v0.16 reconciliation", in: surface.metrics), "passed")
        XCTAssertEqual(metricValue("v0.16 checksum count", in: surface.metrics), "3")
        XCTAssertEqual(metricValue("Boundary", in: surface.metrics), "confirmed")
        XCTAssertTrue(surface.details.contains("Dashboard command surface: none"))
        XCTAssertTrue(surface.details.contains("Trading button: none"))
        XCTAssertTrue(surface.details.contains("Order form: none"))
        ...
        XCTAssertTrue(snapshot.isReadModelOnly)
        XCTAssertTrue(snapshot.viewModelSources.allSatisfy(\.isReadModelOnly))
        XCTAssertTrue(snapshot.smokeSummary.contains("releaseV0160ArtifactRows=3"))
        XCTAssertTrue(snapshot.smokeSummary.contains("releaseV0160ActionSequence=submit,cancel,status"))
        XCTAssertTrue(snapshot.smokeSummary.contains("releaseV0160Reconciliation=passed"))
        XCTAssertTrue(snapshot.smokeSummary.contains("releaseV0160ChecksumCount=3"))
        XCTAssertTrue(snapshot.smokeSummary.contains("releaseV0160ArtifactBoundary=confirmed"))

        let encodedSurface = try JSONEncoder().encode(surface)
        let decodedSurface = try JSONDecoder().decode(
            ReleaseV0160DashboardArtifactBackedExecutionViewModel.self,
            from: encodedSurface
        )
        XCTAssertEqual(decodedSurface, surface)
        ...
        let encoder = JSONEncoder()
        encoder.outputFormatting = [.sortedKeys]
        let artifactJSON = try encoder.encode(ReleaseV0160DashboardArtifactBackedExecutionLocalArtifactInput())
        let artifactInput = try ReleaseV0160DashboardArtifactBackedExecutionViewModel
            .localReadModelArtifactInput(fromJSON: artifactJSON)
        let loadedSurface = try DashboardShellSnapshot
            .releaseV0160DashboardArtifactBackedExecutionView(fromLocalReadModelJSON: artifactJSON)
        let loadedSnapshot = DashboardShellSnapshot(
            viewModel: try makeDashboardViewModel(),
            releaseV0160DashboardArtifactBackedExecutionView: loadedSurface
        )

        XCTAssertTrue(artifactInput.inputHeld)
        XCTAssertEqual(artifactInput.artifactID, "gh-1108-dashboard-artifact-backed-execution-view")
        XCTAssertEqual(
            artifactInput.schema,
            ReleaseV0160DashboardArtifactBackedExecutionLocalArtifactInput.schemaID
        )
        XCTAssertEqual(artifactInput.releaseVersion, "v0.16.0")
        XCTAssertTrue(loadedSurface.boundaryHeld)
        XCTAssertTrue(loadedSnapshot.isReadModelOnly)
        XCTAssertTrue(loadedSnapshot.smokeSummary.contains("releaseV0160ArtifactBoundary=confirmed"))

        let invalidPath = try encoder.encode(
            ReleaseV0160DashboardArtifactBackedExecutionLocalArtifactInput(
                relativePath: "../escape/execution-view.json"
            )
        )
        XCTAssertThrowsError(
            try ReleaseV0160DashboardArtifactBackedExecutionViewModel
                .localReadModelArtifact(fromJSON: invalidPath)
        )

        let commandSurface = try encoder.encode(
            ReleaseV0160DashboardArtifactBackedExecutionLocalArtifactInput(
                surface: ReleaseV0160DashboardArtifactBackedExecutionViewModel(
                    dashboardCommandSurfaceEnabled: true
                )
            )
        )
        XCTAssertThrowsError(
            try DashboardShellSnapshot.releaseV0160DashboardArtifactBackedExecutionView(
                fromLocalReadModelJSON: commandSurface
            )
        )
        ...
        for anchor in ReleaseV0160DashboardArtifactBackedExecutionViewModel.requiredValidationAnchors {
            XCTAssertTrue(surface.validationAnchors.contains(anchor), "\(anchor) must be part of GH-1108 surface")
        }
    }
    ...
    func testGH1283DashboardCLIReadOnlyCanaryStatusSurfaceShowsCanaryEvidenceWithoutCommands() throws {
        // 测试场景：GH-1283 Dashboard / CLI 共用 v0.21.0 canary status surface。
        // 验证目的：Dashboard 和 CLI 只能展示 canary state、gates、risk decision、
        // order lifecycle、cancel / rollback 与 reconciliation；不能出现交易按钮、订单表单、
        // live command、submit / cancel / replace 或 production cutover。
        ...
        let snapshot = DashboardShellSnapshot(viewModel: try makeDashboardViewModel())
        let surface = snapshot.releaseV0210DashboardCLICanaryStatusSurface

        XCTAssertEqual(surface.issueID, "GH-1283")
        XCTAssertEqual(surface.upstreamIssueIDs, ["GH-1280", "GH-1281", "GH-1282"])
        XCTAssertEqual(surface.previousIssueID, "GH-1282")
        XCTAssertEqual(surface.downstreamIssueID, "GH-1284")
        XCTAssertEqual(surface.releaseVersion, "v0.21.0")
        XCTAssertTrue(surface.source.isReadModelOnly)
        XCTAssertTrue(surface.boundaryHeld)
        XCTAssertTrue(surface.surfaceHeld)
        XCTAssertTrue(surface.upstreamEvidenceHeld)
        XCTAssertEqual(surface.rowCount, 7)
        ...
        XCTAssertTrue(surface.rows.allSatisfy { $0.liveCommandVisible == false })
        XCTAssertTrue(surface.rows.allSatisfy { $0.submitCancelReplaceEnabled == false })
        XCTAssertTrue(surface.rows.allSatisfy { $0.rawOrderIDVisible == false })
        XCTAssertTrue(surface.rows.allSatisfy { $0.rawBrokerPayloadVisible == false })
        XCTAssertTrue(surface.rows.allSatisfy { $0.productionCutoverAuthorized == false })
        XCTAssertFalse(surface.productionTradingEnabledByDefault)
        XCTAssertFalse(surface.productionSecretValueRead)
        XCTAssertFalse(surface.productionEndpointConnected)
        XCTAssertFalse(surface.brokerEndpointConnected)
        ...
        XCTAssertEqual(metricValue("v0.21 canary rows", in: surface.metrics), "7")
        XCTAssertEqual(metricValue("v0.21 canary states", in: surface.metrics), "ready,accepted,cancelled,reconciled,fail-closed")
        XCTAssertEqual(
            metricValue("Canary gates", in: surface.metrics),
            "environment,hard-limit,risk-kill-notrade,single-submit-evidence,cancel-rollback,oms-reconciliation,no-command-redaction"
        )
        XCTAssertEqual(
            metricValue("Lifecycle events", in: surface.metrics),
            "submit request,submit accepted,status response,cancel request,cancel outcome,rollback guard,reconciliation"
        )
        XCTAssertEqual(metricValue("Reconciliation", in: surface.metrics), "matched,rejected")
        XCTAssertEqual(metricValue("Boundary", in: surface.metrics), "confirmed")
        XCTAssertTrue(surface.details.contains("Dashboard command surface: none"))
        XCTAssertTrue(surface.details.contains("CLI command surface: read-only status / events / reconciliation only"))
        XCTAssertTrue(surface.details.contains("Trading button: none"))
        XCTAssertTrue(surface.details.contains("Order form: none"))
        XCTAssertTrue(surface.details.contains("Live command: none"))
        XCTAssertTrue(surface.details.contains("Submit / cancel / replace: none"))
        ...
        XCTAssertTrue(snapshot.isReadModelOnly)
        XCTAssertTrue(snapshot.viewModelSources.allSatisfy(\.isReadModelOnly))
        XCTAssertTrue(snapshot.smokeSummary.contains("releaseV0210CanaryRows=7"))
        XCTAssertTrue(snapshot.smokeSummary.contains("releaseV0210CanaryStates=ready,accepted,cancelled,reconciled,fail-closed"))
        XCTAssertTrue(snapshot.smokeSummary.contains("releaseV0210CanaryReconciliation=matched,rejected"))
        XCTAssertTrue(snapshot.smokeSummary.contains("releaseV0210CanaryBoundary=confirmed"))
        ...
        XCTAssertTrue(eventsOutput.contains("mtpro canary-status events"))
        ...
        XCTAssertTrue(eventsOutput.contains("rawOrderIDVisible=false"))
        XCTAssertTrue(eventsOutput.contains("rawBrokerPayloadVisible=false"))
        ...
        XCTAssertTrue(reconciliationOutput.contains("mtpro canary-status reconciliation"))
        ...
        XCTAssertTrue(reconciliationOutput.contains("productionCutoverAuthorized=false"))

        let encodedSurface = try JSONEncoder().encode(surface)
        let decodedSurface = try JSONDecoder().decode(
            ReleaseV0210DashboardCLICanaryStatusSurfaceViewModel.self,
            from: encodedSurface
        )
        XCTAssertEqual(decodedSurface, surface)

        for anchor in ReleaseV0210DashboardCLICanaryStatusSurfaceViewModel.requiredValidationAnchors {
            XCTAssertTrue(surface.validationAnchors.contains(anchor), "\(anchor) must be part of GH-1283 surface")
        }
    }
