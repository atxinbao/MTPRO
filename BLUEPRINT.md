...
后续所有 Project Draft 必须对齐 `MTPRO Core Engine Architecture & Module Maturity Map v1`：说明补的是哪个 Engine / Layer、目标 maturity level、当前 evidence、允许施工范围、forbidden capabilities 和 validation anchors。`MTPRO Event-Driven Paper Trading Runtime v1` 只能被解释为 paper-only L1 起点，不等于完整交易系统，也不授权 Live PRO Console。
