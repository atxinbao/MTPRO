...
| Environment | `docs/environment.md` | 本地工具、验证入口、外部系统禁区 |
| Engineering Module Map / 工程模块地图 | `docs/architecture.md` | 承接 `BLUEPRINT.md`，把蓝图翻译为工程模块、模块边界、数据流、接口、约束和技术分层 |
| Construction Plan | `docs/roadmap.md` | 根据蓝图和工程模块定义施工顺序、Project closure、Current Foundation Progress、Final Product Goal Progress 和下一阶段 handoff |
...
| `docs/architecture.md` | 工程模块地图：承接蓝图后的系统模块、模块边界、数据流、接口、约束和技术分层 | 不重新定义产品目标，不维护完整未来蓝图，不记录 Stage Audit 流水账 |
...
本节承接 Product Blueprint，把最终产品要求翻译为系统结构原则。具体模块边界、数据流、接口、约束和技术分层由 `docs/architecture.md` 继续工程化维护。
...
下一阶段方向仍必须由 Human + `@001 / PLN` 基于本文档、`GOAL.md`、`docs/roadmap.md`、Stage Code Audit Reports 和最新验证摘要确认。
