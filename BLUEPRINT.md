...
Release v0.16.0 operator beta queue anchor：`MTPRO Release v0.16.0 Binance Spot Testnet Operator Execution Beta` 使用 GitHub fallback issues #1101..#1112，WIP=1。#1101 使用 `GH-1101-VERIFY-V0160-OPERATOR-BETA-CONTRACT` 定义 v0.16.0 contract / preflight，现已 closed / done。#1102 使用 `GH-1102-VERIFY-V0160-OPERATOR-RUN-MODEL` 定义 durable run id lifecycle、action sequence、artifact linkage、redacted metadata 和 invalid transition fail-closed guard，现已 closed / done。#1103 使用 `GH-1103-VERIFY-V0160-CLI-SUBMIT-FLOW` 定义 `spot-testnet-submit` stable CLI submit flow，现已 closed / done。#1104 是当前 WIP=1，使用 `GH-1104-VERIFY-V0160-CLI-CANCEL-FLOW` 定义 `spot-testnet-cancel` stable CLI cancel、source submit artifact identity、v0.15.1 guarded runtime delegation、explicit operator confirmation、testnet credential profile、redacted order reference、append-only event evidence 和 missing prior artifact fail-closed guard。#1104 不读取 production secret，不连接 production endpoint / broker endpoint，不提交 production order，不授权 production cutover。
