...
Release v0.15.1 queue anchor：`MTPRO Release v0.15.1 Real Testnet Execution Hardening Patch` 当前由 GitHub fallback issues #1094..#1100 承载。#1094 使用 `GH-1094-VERIFY-V0151-V0150-RELEASE-FACT-SYNC` 同步 v0.15.0 已发布事实并拒绝未限定到 #1076 historical closeout 的 stale publication wording；#1095..#1100 remain backlog / non-executable until dependencies are done。该 hardening queue 不创建下一 Project / Issue，不授权 production cutover，不读取 production secret，不连接 production endpoint / broker endpoint，不提交 production order。
