...
Release v0.15.1 queue anchor：`MTPRO Release v0.15.1 Real Testnet Execution Hardening Patch` 当前由 GitHub fallback issues #1094..#1100 承载。#1094 使用 `GH-1094-VERIFY-V0151-V0150-RELEASE-FACT-SYNC` 同步 v0.15.0 已发布事实并已 closed / done；#1095 使用 `GH-1095-VERIFY-V0151-INJECTED-TRANSPORT-WORDING` 固定 injected transport wording：v0.15.0 提供 signed execution runtime contracts、injected Spot Testnet transport protocol evidence、mock/manual proof split 和 redacted artifacts，不表示内置 URLSession runner、CLI 默认真实联网 runner 或 production broker connector；#1096..#1100 remain backlog / non-executable until dependencies are done。该 hardening queue 不创建下一 Project / Issue，不授权 production cutover，不读取 production secret，不连接 production endpoint / broker endpoint，不提交 production order。
