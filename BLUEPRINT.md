...
Release v0.15.0 queue anchor：`MTPRO Release v0.15.0 Real Binance Testnet Execution MVP` 已通过 #1066 定义 contract / v0.14.1 preflight gate。`GH-1066-VERIFY-V0150-CONTRACT-PREFLIGHT` 将 v0.15.0 MVP 限定为 Binance Spot Testnet only，productionTradingEnabledByDefault=false，operatorConfirmationRequired=true，testnetEndpointAllowlistOnly=true；#1067 通过 `GH-1067-VERIFY-V0150-TESTNET-CREDENTIAL-SIGNED-REQUEST` 固定 credential reference、HMAC-SHA256 signed request construction、redacted evidence、production endpoint blocked 和 no network action；#1068 通过 `GH-1068-VERIFY-V0150-REAL-SPOT-TESTNET-SUBMIT-RUNTIME` 固定 OrderIntent -> signed Spot Testnet submit request -> injected transport -> redacted response evidence 的 guarded submit runtime，`testnetNetworkSubmitPerformed=true`，`appendOnlyEvidenceCreated=true`，production endpoint / broker endpoint / production order 全部保持 false；#1071 通过 `GH-1071-VERIFY-V0150-NETWORK-EXECUTION-EVENT-LOG` 固定 append-only redacted checksum network event log、request / response identity、checksum chain 和 rawSecretPersisted=false；#1069 通过 `GH-1069-VERIFY-V0150-REAL-SPOT-TESTNET-CANCEL-RUNTIME` 固定 prior submit evidence -> signed Spot Testnet cancel request -> injected cancel transport -> OMS cancel transition -> append-only cancel event 的 guarded cancel runtime，`testnetNetworkCancelPerformed=true`，`appendOnlyCancelEvidenceCreated=true`，`omsStateTransitionIntegrated=true`，production endpoint / broker endpoint / production order 全部保持 false；#1070 通过 `GH-1070-VERIFY-V0150-REAL-SPOT-TESTNET-CANCEL-REPLACE-RUNTIME` 固定 native cancel-replace fail-closed、cancel + new submit emulation、OMS replace transition 和 aggregate append-only cancel-replace event，`nativeCancelReplaceSupported=false`，`nativeReplaceRejectedFailClosed=true`，`cancelThenNewSubmitEmulationUsed=true`，production endpoint / broker endpoint / production order 全部保持 false；#1072 通过 `GH-1072-VERIFY-V0150-OMS-STATE-SYNC-RECONCILIATION` 固定从 append-only network event log 生成本地 OMS state snapshot 和 expected / observed reconciliation report，`derivedFromNetworkEventLogOnly=true`，`appendOnlyNetworkExecutionEventLog=true`，`expectedObservedReconciliation=true`，`mismatchesFailClosed=true`，raw broker payload / broker fill / production order 全部保持 false；当前 WIP=1 issue #1073 通过 `GH-1073-VERIFY-V0150-CLI-OPERATOR-FLOW` 固定 `mtpro testnet-execution` 的 explicit testnet mode、operator confirmation、redacted output、no production fallback 和 append-only evidence reference。该 queue 不授权 production cutover，不读取 production secret，不连接 production endpoint / broker endpoint，不提交 production order。
