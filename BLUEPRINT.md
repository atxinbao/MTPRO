...
Release v0.16.0 operator beta queue anchor：`MTPRO Release v0.16.0 Binance Spot Testnet Operator Execution Beta` 使用 GitHub fallback issues #1101..#1112，WIP=1。#1101 使用 `GH-1101-VERIFY-V0160-OPERATOR-BETA-CONTRACT` 定义 v0.16.0 contract / preflight，现已 closed / done。#1102 是当前 WIP=1，使用 `GH-1102-VERIFY-V0160-OPERATOR-RUN-MODEL` 定义 durable run id lifecycle、action sequence、artifact linkage、redacted metadata 和 invalid transition fail-closed guard。#1102 不读取 testnet credential value，不连接 testnet endpoint，不提交 testnet order，不授权 production cutover。
