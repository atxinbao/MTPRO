...
当前 blocker 包含：仍需继续证明 retained compatibility envelopes 的退出路径；`Core`、`Adapters`、`Persistence` 和 `Runtime` 不能被写成最终 architecture module owner。GH-392 已移除 `Trader -> ExecutionEngine` direct target dependency；GH-420 已清理 Dashboard active source 的 Workbench naming residue。
...
GH-419 把 Database / Persistence / Runtime 的当前 ownership 口径收紧成一张显式矩阵：`Database` target 现在直接编译 `DatabaseRuntimeOwnershipMatrix.swift`、`FoundationDatabaseCheckpoint.swift` 和 `TargetGraph/DatabaseTargetBoundary.swift`，用于表达 durable boundary、local projection vocabulary 和 current ownership status。它不把 SQLite / DuckDB projection adapter 或 replay / ingest workflow 误称为已经完全迁入 Database。

`GH-419-PERSISTENCE-CORE-DEPENDENCY-DEFERRED-ONLY`

`Persistence` 仍是 retained compatibility envelope，只承载 `Sources/Database/Projections/SQLite/Persistence.swift` 和 `Sources/Database/Projections/DuckDB/DuckDBAnalyticalProjectionAdapter.swift`。保留原因是这些 adapter 仍依赖 `Core` 中的 rich `EventEnvelope`、paper / risk / portfolio projection payload 和 legacy compatibility surface。它们的 exit gate 是先把 neutral event payload / projection DTO 拆到真实 `MessageBus` / `Database` source root，再逐段退休 `Persistence` envelope。

`GH-419-RUNTIME-REPLAY-INGEST-COMPOSITION-ONLY`

`Runtime` 仍是 retained composition envelope，只承载 `Sources/Database/ReplayProjection/` 和 `Sources/DataEngine/Ingest/` 的跨模块 workflow。它不作为 Database 或 DataEngine 的最终 implementation owner，也不变成 UI / live / broker runtime。后续只有在 replay projection 与 ingest workflow 的依赖被拆成中立 contracts 后，才能迁回真实 module source roots。

`GH-419-NO-SCHEMA-RUNTIME-BROKER-L4-GUARD`

GH-419 不暴露 SQLite / DuckDB schema 给 Dashboard，不暴露 Runtime object、Adapter request、account payload、broker payload 或 broker state，不实现 Trader runtime、Strategy runtime、Live runtime、ExecutionClient implementation、OMS、broker gateway、signed/account endpoint、private stream runtime、real order lifecycle、Live PRO Console、trading button、live command、order form 或 L4 capability。

## GH-420 Dashboard Read-model-only Active Naming Cleanup

`GH-420-DASHBOARD-ACTIVE-SOURCE-NAMING-CLEAN`

GH-420 将 active Dashboard source 的当前命名收口到 `Dashboard read-model-only boundary`：

- `Sources/Dashboard/` active source 不再使用 `Workbench` / `workbench` 命名。
- `DashboardTargetBoundary` 增加 `GH-420-DASHBOARD-ACTIVE-SOURCE-NAMING-CLEAN` validation anchor。
- `LiveReadOnlyDashboardBoundaryReadModel` 通过 `Core.LiveReadOnlyDashboardReadModelBoundary` 消费历史 MTP-131 read-only contract，避免 Dashboard 模块直接引用旧 Workbench active surface 名称。
- `DashboardBetaAcceptancePath` 的 current evidence id 改为 `mtp-122-dashboard-beta-acceptance-path`。
...
Target System Architecture 的工程分层压缩为五层。依赖方向从 Dashboard 往下读取稳定边界；事实流从输入源进入 DataClient / DataEngine 后写入 MessageBus / Event Log，再通过 replay / projection / read model 反向供 Dashboard 展示。

```mermaid
flowchart TB
    L1["Dashboard UI Layer<br/>SwiftUI / macOS shell"]
...
| --- | --- | --- | --- | --- | --- |
| Dashboard UI Layer | SwiftUI / macOS shell、页面布局、只读展示、本地 Paper session-level control | 禁止 UI trading button、live command、DB schema、adapter / runtime direct access | App Interface | Human 用户 | Current |
| App Interface Layer | ViewModel assembly、Command Model boundary、Report / Dashboard / Event Timeline app contract | 禁止领域规则、broker action、Binance direct call、持久化事实 | Evidence Read Model | Dashboard UI | Current |
...
`GH-413-CORE-ENVELOPE-RETIREMENT-CONTRACT` 固定下一轮 architecture graph completion 的验收口径：目录、SwiftPM target 和 `TargetGraph` boundary anchor 已经存在，不等于真实 implementation ownership 已完成。后续必须逐模块证明 real source root 能独立承载核心实现，并把 `Core`、`Adapters`、`Persistence`、`Runtime` 收回到明确的 compatibility shim / re-export 角色。

`GH-413-REAL-MODULE-OWNERSHIP-ACCEPTANCE-CRITERIA`：

- `MessageBus` 必须拥有中立 command / event / request-response spine、append-only journal 和 proposal / risk decision contracts；rich paper routing payload 不能长期由 `Core` primary ownership 承载。
- `DataEngine` 必须拥有 ingest / replay / quality contracts；`ScenarioReplay`、`DataQuality` 和 `Ingest` 不能长期依赖 `Core` / `Runtime` 作为 primary implementation owner。
- `Database` 必须拥有 projection / replay projection / checkpoint boundary；Dashboard 不得读取 schema。
- `Portfolio`、`RiskEngine`、`ExecutionEngine` 必须分别拥有 financial projection、pre-execution risk gate、paper / simulated lifecycle；`ExecutionClient` 仍只是 future gate / protocol boundary，不是 broker implementation。
- `Trader` 必须保持 `Accounts + Strategies/EMA + Coordination`，不直接拥有 `ExecutionEngine`、`ExecutionClient`、broker、OMS、live command 或 UI command surface。
- `Dashboard` 必须保持 read-model-only boundary，不恢复 `Workbench` / `AppCompatibility` active modules。

`GH-413-SOURCE-ROOT-BOUNDARY-ANCHOR-FUTURE-GATE-MATRIX` 要求后续每个 source move / target edit 都明确区分：

- real module source root：真实 implementation 归属地。
- boundary anchor：`Sources/*/TargetGraph/*TargetBoundary.swift`，只证明 target shape，不证明完整 ownership。
- retained compatibility envelope：`Core`、`Adapters`、`Persistence`、`Runtime`，只能保留明确原因和退出路径。
- future gate：`ExecutionClient`、OMS、broker gateway、live command、L4 production trading，只表达未来能力边界。

`GH-413-NO-L4-RUNTIME-BROKER-GUARD`：本轮 Core envelope retirement 仍不授权 Trader runtime、Strategy runtime、Live runtime、ExecutionClient implementation、OMS、broker gateway、signed endpoint、account endpoint / listenKey、private WebSocket runtime、real order lifecycle、Live PRO Console、trading button、live command、order form 或 L4 implementation。
