...
当前 GitHub fallback queue 为 `release/v0.15.1` issues `#1094..#1100`。#1094 是 WIP=1：`GH-1094-VERIFY-V0151-V0150-RELEASE-FACT-SYNC`，只同步 v0.15.0 已发布事实、拒绝未限定到 #1076 historical closeout 的 stale publication wording，并保持 no-production boundary。#1095..#1100 remain backlog / non-executable until dependencies are done。v0.15.1 是 v0.15.0 后的 hardening patch queue，不创建下一 Project / Issue，不授权 production cutover，不读取 production secret，不连接 production endpoint / broker endpoint，不提交 production order。
...
| Current v0.15.1 queue | `MTPRO Release v0.15.1 Real Testnet Execution Hardening Patch`；current issue `#1094`；`GH-1094-VERIFY-V0151-V0150-RELEASE-FACT-SYNC`；`#1095..#1100` remain backlog / non-executable until dependencies are done |
...
当前 GitHub fallback queue 为 `release/v0.15.1` issues `#1094..#1100`。当前 WIP=1 是 #1094 release fact sync guard；#1095..#1100 在 #1094 PR merge、required checks SUCCESS、issue closed / done、main fast-forward 和 worktree clean 前保持 backlog / non-executable。
