...
| Current queue | `MTPRO Release v0.15.0 Real Binance Testnet Execution MVP` active as GitHub fallback queue；#1066 is the current WIP=1 contract / v0.14.1 preflight gate；#1067..#1076 remain backlog / non-executable |
...
- 当前 GitHub fallback `release/v0.15.0` queue 为 #1065 至 #1076：#1066 使用 `GH-1066-VERIFY-V0150-CONTRACT-PREFLIGHT` 定义 release contract、v0.14.1 preflight gate、Binance Spot Testnet only boundary 和 no-production fail-closed gate；#1067..#1076 remain backlog / non-executable until #1066 PR merge / checks SUCCESS / issue done。v0.15.0 MVP 不授权 production cutover，不读取 production secret，不连接 production endpoint / broker endpoint，不提交 production order。
