...
| Current queue | `MTPRO Release v0.15.0 Real Binance Testnet Execution MVP` active as GitHub fallback queue；#1066 contract / v0.14.1 preflight gate is closed / done；#1067 testnet credential / signed request builder gate is closed / done；#1068 real Spot Testnet submit runtime gate is closed / done；#1071 network execution event log gate is closed / done；#1069 real Spot Testnet cancel runtime gate is closed / done；#1070 real Spot Testnet cancel-replace runtime gate is closed / done；#1072 OMS state sync + reconciliation gate is closed / done；#1073 is the current WIP=1 CLI operator flow gate；#1074..#1076 remain backlog / non-executable |
...
- 当前 GitHub fallback `release/v0.15.0` queue 为 #1065 至 #1076：#1066 使用 `GH-1066-VERIFY-V0150-CONTRACT-PREFLIGHT` 定义 release contract、v0.14.1 preflight gate、Binance Spot Testnet only boundary 和 no-production fail-closed gate；#1067 使用 `GH-1067-VERIFY-V0150-TESTNET-CREDENTIAL-SIGNED-REQUEST` 固定 Binance Spot Testnet credential reference、短生命周期 credential material、HMAC-SHA256 signed request construction evidence、production secret auto-read blocked、production endpoint blocked 和 no network action；#1068 使用 `GH-1068-VERIFY-V0150-REAL-SPOT-TESTNET-SUBMIT-RUNTIME` 固定 OrderIntent -> signed Spot Testnet submit request -> injected transport -> redacted response evidence 的 guarded submit runtime，`testnetNetworkSubmitPerformed=true` 且 production endpoint / broker endpoint / production order 全部保持 false；#1071 使用 `GH-1071-VERIFY-V0150-NETWORK-EXECUTION-EVENT-LOG` 固定 append-only redacted checksum network event log、request / response identity 和 rawSecretPersisted=false；#1069 使用 `GH-1069-VERIFY-V0150-REAL-SPOT-TESTNET-CANCEL-RUNTIME` 固定 prior submit evidence -> signed Spot Testnet cancel request -> injected cancel transport -> OMS cancel transition -> append-only cancel event 的 guarded cancel runtime，`testnetNetworkCancelPerformed=true`、`appendOnlyCancelEvidenceCreated=true`、`omsStateTransitionIntegrated=true` 且 production endpoint / broker endpoint / production order 全部保持 false；#1070 使用 `GH-1070-VERIFY-V0150-REAL-SPOT-TESTNET-CANCEL-REPLACE-RUNTIME` 固定 native cancel-replace fail-closed、cancel + new submit emulation、OMS replace transition 和 aggregate append-only cancel-replace event，`nativeCancelReplaceSupported=false`、`nativeReplaceRejectedFailClosed=true`、`cancelThenNewSubmitEmulationUsed=true` 且 production endpoint / broker endpoint / production order 全部保持 false；#1072 使用 `GH-1072-VERIFY-V0150-OMS-STATE-SYNC-RECONCILIATION` 固定从 #1071 append-only network event log 推导本地 OMS state snapshot 和 expected / observed reconciliation report，`derivedFromNetworkEventLogOnly=true`、`appendOnlyNetworkExecutionEventLog=true`、`expectedObservedReconciliation=true`、`mismatchesFailClosed=true` 且 raw broker payload / broker fill / production order 全部保持 false；当前 #1073 使用 `GH-1073-VERIFY-V0150-CLI-OPERATOR-FLOW` 固定 `mtpro testnet-execution` 的 explicit testnet mode、operator confirmation、redacted output、no production fallback 和 append-only evidence reference。v0.15.0 MVP 不授权 production cutover，不读取 production secret，不连接 production endpoint / broker endpoint，不提交 production order。
