...
- 当前 live queue source 为 GitHub fallback `release/v0.13.0`，#994 contract gate 已完成；#995 是唯一 active local evidence intake model gate；#996..#1005 保持 blocked，直到 #995 PR merged / required checks success / issue closed done / main fast-forward / fresh WIP=1 preflight。Release v0.12.0 completion、v0.12.1 patch closeout、v0.13.0 contract gate 和 #995 local evidence intake model 不授权 strategy 直连 ExecutionClient、broker command、production OMS、trading button、Live PRO Console production command、live command、real broker、real order、testnet order routing、production cutover 或任何 production trading。
