...
| Current queue | `MTPRO Release v0.15.1 Real Testnet Execution Hardening Patch` active as GitHub fallback queue；#1094 release fact sync guard is current WIP=1；#1095..#1100 remain backlog / non-executable until dependencies are done |
...
- 当前 GitHub fallback `release/v0.15.1` queue 为 #1094 至 #1100：#1094 使用 `GH-1094-VERIFY-V0151-V0150-RELEASE-FACT-SYNC` 同步 v0.15.0 publication facts 并拒绝未限定到 #1076 historical closeout 的 stale publication wording；#1095..#1100 remain backlog / non-executable until dependencies are done。
