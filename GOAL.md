...
- 当前 live queue source 为 GitHub fallback `release/v0.13.0`，#994 contract gate、#995 local evidence intake model gate 和 #996 synthetic provenance rejection gate 已完成；#997 是唯一 active build pipeline gate；#998..#1005 保持 blocked，直到 #997 PR merged / required checks success / issue closed done / main fast-forward / fresh WIP=1 preflight。Release v0.12.0 completion、v0.12.1 patch closeout、v0.13.0 contract gate、#995 local evidence intake model、#996 synthetic provenance rejection 和当前 #997 build pipeline 不授权 strategy 直连 ExecutionClient、broker command、production OMS、trading button、Live PRO Console production command、live command、real broker、real order、testnet order routing、production cutover 或任何 production trading。
