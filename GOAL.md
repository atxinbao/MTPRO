...
| Current v0.16.0 operator beta queue | `MTPRO Release v0.16.0 Binance Spot Testnet Operator Execution Beta` uses GitHub fallback queue #1101..#1112；#1101 contract / preflight is current WIP=1 and uses `GH-1101-VERIFY-V0160-OPERATOR-BETA-CONTRACT` to prove v0.15.1 / #1100 dependency, Binance Spot Testnet-only boundary, explicit operator confirmation, redacted evidence, queue order and no-production-cutover guard；#1101 does not read credential value, does not connect testnet endpoint, does not submit testnet order and does not authorize production cutover |
