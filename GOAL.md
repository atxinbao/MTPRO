...
| Current queue | `MTPRO Release v0.15.1 Real Testnet Execution Hardening Patch` active as GitHub fallback queue；#1094 release fact sync guard closed / done；#1095 injected transport wording guard is current WIP=1；#1096..#1100 remain backlog / non-executable until dependencies are done |
...
- 当前 GitHub fallback `release/v0.15.1` queue 为 #1094 至 #1100：#1094 使用 `GH-1094-VERIFY-V0151-V0150-RELEASE-FACT-SYNC` 同步 v0.15.0 publication facts 并已 closed / done；#1095 使用 `GH-1095-VERIFY-V0151-INJECTED-TRANSPORT-WORDING` 明确 v0.15.0 是 signed execution runtime contracts + injected Spot Testnet transport protocol evidence，不是仓库内置 URLSession runner、CLI 默认真实联网 runner 或 production broker connector；#1096..#1100 remain backlog / non-executable until dependencies are done。
