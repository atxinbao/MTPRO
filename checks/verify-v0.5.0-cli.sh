...
require_output_contains "$help_output" "commands=help,run,status,stop,recover,risk-policy,readiness,monitor,verify,testnet-execution,spot-testnet-submit,spot-testnet-cancel,spot-testnet-status-query,operator-run,rehearsal-status,unified-run-status,run-observer,run-detail-observer,testnet-readonly-probe,verify-fast,verify-release"
