...
require_output_contains "$help_output" "runtimeModes=local-dry-run,testnet-read-only-probe,production-blocked"
...
probe_output="$(swift run mtpro run --mode testnet-read-only-probe)"
require_output_contains "$probe_output" "mode=testnet-read-only-probe"
