...
- `MTPRO Account / Position / Balance Read-model-only v1` 已完成的 L3.1 APB evidence 只代表 account / position / balance terminology、snapshot identity、source / freshness evidence、position exposure evidence、balance paper-vs-real boundary、deterministic fixture、forbidden real account tests 和 Workbench / Report / Events read-model-only surface 已闭环；不代表 required validation 需要 secret，不代表 account / position / balance runtime、account snapshot runtime、private WebSocket runtime、real account read、broker position sync、real balance、margin、leverage、real PnL runtime、signed endpoint、account endpoint / listenKey、broker adapter、`LiveExecutionAdapter`、OMS、Live PRO Console、trading button 或 live command 已实现或获授权。
- `MTPRO Private Stream / Account Snapshot Simulation Gate v1` 已完成的 L3.2 simulation gate evidence 只代表 simulated private account event source identity、simulated account snapshot input、account snapshot update fixture、fresh / stale / blocked / missing evidence、forbidden endpoint / runtime tests 和 Workbench / Report / Events read-model-only simulation gate surface 已闭环；不代表 required validation 需要 secret，不代表 private stream runtime、account snapshot runtime、Live read-only runtime、signed endpoint、account endpoint / listenKey、private WebSocket runtime、broker adapter、`LiveExecutionAdapter`、OMS、Live PRO Console、trading button 或 live command 已实现或获授权。
