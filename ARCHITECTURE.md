...
当前已完成 Research -> Backtest -> Report -> Paper readiness、paper-only execution evidence，以及本地 Paper workflow 可观察性和 session-level control shell。MTPRO 仍不进入 Live trading。
...
| `App` | Trader Workstation Dashboard 产品面和 ViewModel 边界；当前包含 Paper workflow observability、Event Timeline / Evidence Explorer read model 和 Dashboard / Workbench shell snapshot |
| `Dashboard` | SwiftPM 可构建 / smoke-run 的 macOS 只读看板 shell，只装载 App 层 ViewModel snapshot；当前展示 read-model-only Workbench、`start` / `pause` / `close` / `reset` session-level local controls 和 evidence preview |
